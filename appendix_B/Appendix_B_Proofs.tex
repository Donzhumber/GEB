...
% !TEX root = Appendix_B_Proofs.tex
% !TEX outputDirectory = out
% =========================================================================
% APPENDIX B -- FORMAL PROOFS AND LEAN VERIFICATION
% Formal proofs supporting the identification argument. Replication: Appendix_B_Proofs/
% =========================================================================
...
\newcommand{\tabsource}{\ \textit{Source:} Analytical proofs in this supplement;
  Lean identifiers in \texttt{lean/AppendixBProofs.lean}. Software: Lean~4
  (\texttt{lean-toolchain}); Mathlib via Lake.}

\begin{document}

\title{Supplement to ``Identifying Rational Types in Unknown Environments''\\
Appendix B: Formal Proofs and Lean Verification}
\author{Humberto Bernal\thanks{Economist, professor of the Economics Program at the Universidad Colegio Mayor de Cundinamarca and graduate of the Ph.D.\ in Economics from the Universidad de los Andes, Colombia. E-mail: \href{mailto:hbernal@uniandes.edu.co}{hbernal@uniandes.edu.co}, \href{mailto:hbernalc@universidadmayor.edu.co}{hbernalc@universidadmayor.edu.co}. ORCID: \href{https://orcid.org/0000-0002-4864-1726}{0000-0002-4864-1726}.}}
\date{\today}
\maketitle
...
seven sections that mirror the logical structure of the model: public information and Bayesian learning
...
are reproduced from the \texttt{Appendix\_B\_Proofs/} replication package
...
$\ell_u(\theta';J)$ induced by the same PBE.
...
determines the observable results of the episode. The episode horizon is the
endogenous stopping time
\begin{equation}
\label{eq:stopping-time}
\tau:=\inf\{t\ge 0:m_t\in\mathcal{M}_{\mathrm{term}}\},
\end{equation}
where $m_t$ is the physical outcome,
...
(fully formalized in Definition~\ref{def:desenlace-fisico-mt}). Active
periods satisfy $t<\tau$ while captivity continues.

% ---------------------------------------------------------------------------

\begin{definition}[MDG stochastic implementation filter]
\label{def:mano-dios-filtro}
Let $i\in\{K,S,F\}$ and let $\mathcal{A}^i$ be the finite set of admissible
actions of agent~$i$. The \textbf{state vector} is
$X_t:=(\mu_t,h_t)$, with $\mu_t$ from
Definition~\ref{def:beliefs-martingale} and $h_t$ the public history at the
opening of~$t$. Given the plan $a_t^{i\ast}\in\mathcal{A}^i$ and $X_t$,
execution is determined by the \textbf{implementation law}
\[
\tilde{a}_t^i \sim \ProbI{i}\bigl(\tilde{a}_t^i \mid a_t^{i\ast},\,X_t\bigr).
\]
In the base logit specification~\eqref{eq:logit-hybrid},
$\ProbI{i}(\tilde{a}_t^i\mid a_t^{i\ast},X_t)$ depends on $X_t$ only through $\mu_t$
(via the temperature $T_t$); conditioning on $h_t$ is reserved for extended
specifications. The stochastic structure is formalized on a measurable space
...
\begin{enumerate}
  ...
  In the \textbf{base specification} with $\underline{c}>0$, for all
  $t<\tau$ (with $\tau$ as in~\eqref{eq:stopping-time}),
  $\ProbI{i}$ satisfies strict \textbf{plan dominance},
  $\ProbI{i}(a_t^{i\ast}\mid a_t^{i\ast},X_t)>\ProbI{i}(a\mid a_t^{i\ast},X_t)$
  for all $a\neq a_t^{i\ast}$, and \textbf{full support},
  $\ProbI{i}(\tilde{a}_t^i=a\mid a_t^{i\ast},X_t)>0$ for all
  $a\in\mathcal{A}^i$.
  \item[\textup{(ii)}] \textbf{Bounded institutional maturation.}
  The informational component
  $[H(\mu_t)/H(\mu_0)]e^{-\eta_{\mathrm{cal}}t}$ declines as posterior
  uncertainty falls, while the structural floor $\underline{c}$ persists in
  ~\eqref{eq:temperatura-piso}. When $\underline{c}>0$, even if
  $H(\mu_t)\to 0$ one has $T_t\ge T_0\underline{c}$ and
  $\ProbI{i}(\tilde{a}_t^i=a_t^{i\ast}\mid a_t^{i\ast},X_t)<1$, so full
  support over $\mathcal{A}^i$ is preserved in the long run.
\end{enumerate}
...
\end{equation}
where $T_0>0$ is the initial temperature, $\eta_{\mathrm{cal}}>0$ is the
calendar decay rate, Shannon entropy is
$H(\mu_t)=-\sum_\theta\mu_t(\theta)\ln\mu_t(\theta)$ with
$H(\mu_0)>0$ by the full-support prior in
Definition~\ref{def:beliefs-martingale}, and
$\underline{c}\in[0,1]$ is the floor of structural noise.
\end{definition}

\begin{remark}[Residual noise and the limit $\underline{c}=0$]
\label{rem:mdg-floor}
The special case $\underline{c}=0$ is interpreted as a limit without residual
noise: it allows
$\ProbE(\tilde{a}_t^i=a_t^{i\ast}\mid\mathcal{F}_t^{\mathrm{pub}})\to 1$
when $H(\mu_t)\to 0$, but sacrifices the persistent full support maintained
under $\underline{c}>0$. The base version with $\underline{c}>0$ aligns with
the absolute continuity or ``grain of truth'' condition of
\citet{KalaiLehrer1993}, according to which rational learning must not assign
zero probability to events that can occur.
\end{remark}
...
\begin{enumerate}
  ...
  probability $1-\varepsilon_t\in(0,1)$, where
  $\varepsilon_t:=1-\ProbI{i}(a_t^{i\ast}\mid a_t^{i\ast},X_t)$ is the
  implementation-error rate induced by the logit.
\end{enumerate}
In the parametrization~\eqref{eq:logit-hybrid}, $\varepsilon_t$ is decreasing
in $1/T_t$. By the characterization theorem of \citet{Jaynes1957Information}, the unique
...
\[
Q_t(m\mid\mathcal C_t):=\ProbE(m_t=m\mid\mathcal C_t)
\]
...
\[
Q_t(\mathrm{cont}\mid\mathcal{C}_t)
...
m_t=r
\iff
\sum_{\ell<r}Q_t(\ell\mid\mathcal{C}_t)\le v_t
<\sum_{\ell\le r}Q_t(\ell\mid\mathcal{C}_t),
\]
...
Let $\tau$ be as in~\eqref{eq:stopping-time}. The
...
Fixing a policy trajectory $(\chi_u)_{u\ge 0}$, suppose that
...
\begin{remark}[Informational Slicing and Conditional Independence]\label{rem:info-slicing}
A crucial conceptual distinction must be maintained between the adaptive
equilibrium path of the active dual control policy and the measure-theoretic
construction required by Lemma~\ref{lem:hazard-separation}
\citep{Kakutani1948, BlackwellDubins1962}. In the live perfect Bayesian
equilibrium, the planner updates beliefs sequentially, causing the realized
instruments $(\alpha_t^*, \gamma_t^*)$ to be non-trivially coupled with the
historical filtration $\{\mathcal{F}_t^{\mathrm{pub}}\}$
\citep{AghionBoltonHarrisJullien1991, EasleyKiefer1988}. However, the
probability measures $\ProbC{\theta}$ invoked in
Definition~\ref{def:asymptotic-id} and Lemma~\ref{lem:hazard-separation}
operate on a frozen deterministic policy path $(\chi_u)_{u \ge 0}$ evaluated
ex-ante at $t=0$ \citep{BlackwellDubins1962}. Within this sliced
counterfactual regime, the strategic intentions of the private agents are
uniquely pinned down by sequential rationality, rendering the daily innovations
of the physical-institutional block $Y_t = (m_t, d_t)$ conditionally
independent across periods \citep{Kakutani1948}. This conditional factorization
satisfies the orthogonal product-measure structure
$\bigl(\bigotimes_{t \ge 0} p_{\theta}^{(t)}\bigr)$ required to validly
operationalize the singularity criterion of \citet{Kakutani1948} without
violating the temporal feedback loop of the dynamic game.
\end{remark}
...
Conditional on the fixed policy path and on $\mathcal{T}$---under the
informational slicing regime clarified in Remark~\ref{rem:info-slicing}---the
law of the sequence factors exactly as a product of daily independent experiments
$\bigotimes_{t\ge 0}p_\theta^{(t)}$ versus
$\bigotimes_{t\ge 0}p_{\theta_K^\ast}^{(t)}$
\citep{Kakutani1948}. Part~(ii) then follows directly from
...
By on-path Bayesian consistency of the PBE (condition~(b) of Definition~\ref{def:pbe-mechanism}) and Definition~\ref{def:beliefs-martingale}, the belief process is represented as
...
bounded martingale under $\ProbE$ with limit $Z_\theta\in[0,1]$ $\ProbE$-a.s. The
...
the prolonged captivity event $\mathcal{T}$ (Definition~\ref{def:event-T-long}).
...
By the Bayesian updating rule defined in Definition~\ref{def:beliefs-martingale} with a full-support prior, the posterior belief satisfies
\[
\mu_t(\theta)
...
Fix a type $\theta_K$ and a period $t$. The following standing assumptions
govern the comparative static in $\gamma_t^\ast$.

\medskip
\noindent\textit{Standing assumptions.}
\textup{(S0)} Baseline hazards satisfy $\lambda_{j0}(s)>0$ for all strategic
causes $j\in\{1,2,3\}$ and all $s$ in the support, so
$\mathbb{E}[\tau\mid\theta_K]<\infty$.
\textup{(S1)} The object $\partial\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast$
is a partial derivative: only $\gamma_t^\ast$ is perturbed at period $t$ while
$\{\gamma_u^\ast\}_{u\neq t}$ is held fixed. Writing
$S(u\mid\theta_K):=\prod_{r=1}^{u}p_{\mathrm{Cont},r\mid\theta_K}$ with
$S(0\mid\theta_K)=1$, expected duration admits the survival representation
\begin{equation}
\label{eq:duracion-identidad-prop}
\mathbb{E}[\tau\mid\theta_K]=\sum_{u=0}^{\infty}S(u\mid\theta_K),
\end{equation}
so only factors with $u\ge t$ depend on $\gamma_t^\ast$.
\textup{(S2)} For each strategic cause $j\in\{1,2,3\}$,
\[
\frac{\partial\tilde\lambda_j(t\mid\theta_K)}{\partial\gamma_t^\ast}
=\eta_{\gamma,j}\,\tilde\lambda_j(t\mid\theta_K),
\qquad
\eta_{\gamma,j}:=\frac{\partial\ln\tilde\lambda_j(t\mid\theta_K)}{\partial\gamma_t^\ast};
\]
the exogenous cause $j=4$ is invariant to $\gamma_t^\ast$.

\medskip
\noindent\textit{Net sensitivity.}
In the main-paper hazard index, payment ($j=1$) is decreasing in $\gamma$ and
death/state rescue ($j\in\{2,3\}$) are increasing, so $\eta_{\gamma,1}<0$ and
$\eta_{\gamma,j}>0$ for $j\in\{2,3\}$. Let $\zeta_{\gamma,j}:=|\eta_{\gamma,j}|$
and define
\begin{equation}
\label{eq:kappa-c}
\kappa_h(\theta_K,t)
:=\zeta_{\gamma,2}\,\tilde\lambda_2(t\mid\theta_K)
 +\zeta_{\gamma,3}\,\tilde\lambda_3(t\mid\theta_K)
 -\zeta_{\gamma,1}\,\tilde\lambda_1(t\mid\theta_K)
=\sum_{j=1}^{3}\eta_{\gamma,j}\,\tilde\lambda_j(t\mid\theta_K).
\end{equation}
...
Under~\textup{(S0)--(S2)},
\begin{equation}
\label{eq:dE-dgamma-prop}
\frac{\partial\,\mathbb{E}[\tau\mid\theta_K]}{\partial\gamma_t^\ast}
=-\kappa_h(\theta_K,t)\,\Delta t\sum_{u=t}^{\infty}S(u\mid\theta_K).
\end{equation}
...
\end{equation}
If $\kappa_h(\theta_K,t)>0$, the one-shot perturbation in~\textup{(S1)} lowers
expected duration; if $\kappa_h(\theta_K,t)<0$, it raises it. Cross-type
variation in the sign enters through $\beta_{K,j}(\theta_K)$ and the induced
semi-elasticities $\{\eta_{\gamma,j}\}$.
\end{proposition}

\begin{proof}
The argument has three steps: differentiate the daily continuation probability,
propagate the perturbation through the survival product, and differentiate the
duration identity~\eqref{eq:duracion-identidad-prop}.

\paragraph{Step 0. Survival representation.}
For a discrete-time competing-risks process with daily continuation probability
$p_{\mathrm{Cont},s\mid\theta_K}$, the survival function
$S(t'\mid\theta_K)=\prod_{s=1}^{t'}p_{\mathrm{Cont},s\mid\theta_K}$ satisfies
$S(t'\mid\theta_K)=\Prob(\tau>t'\mid\theta_K)$. Hence
\[
\mathbb{E}[\tau\mid\theta_K]
=\sum_{t'=0}^{\infty}\Prob(\tau>t'\mid\theta_K)
=\sum_{t'=0}^{\infty}S(t'\mid\theta_K),
\]
which is the discrete-time survival identity (Wald's representation; see, e.g.,
\citet{KalbfleischPrentice2002}). Finiteness in~\textup{(S0)} makes the sum
well defined.

\paragraph{Step 1. Daily continuation and $\kappa_h$.}
By Definition~\ref{def:desenlace-fisico-mt},
\[
p_{\mathrm{Cont},s\mid\theta_K}
=\exp\!\Bigl(-\sum_{j=1}^{4}\tilde\lambda_j(s\mid\theta_K)\,\Delta t\Bigr)
=\exp\!\bigl(-[\Lambda_s(\theta_K)+\tilde\lambda_4(s)]\,\Delta t\bigr),
\]
where $\Lambda_s(\theta_K):=\sum_{j=1}^{3}\tilde\lambda_j(s\mid\theta_K)$ is the
strategic hazard total. Only $\Lambda_t$ depends on $\gamma_t^\ast$; the
exogenous term $\tilde\lambda_4(t)$ is $\gamma$-invariant. By~\textup{(S2)},
\[
\frac{\partial\Lambda_t(\theta_K)}{\partial\gamma_t^\ast}
=\sum_{j=1}^{3}\eta_{\gamma,j}\,\tilde\lambda_j(t\mid\theta_K)
=\kappa_h(\theta_K,t),
\]
using $\eta_{\gamma,1}=-\zeta_{\gamma,1}$ and $\eta_{\gamma,j}=+\zeta_{\gamma,j}$
for $j\in\{2,3\}$ in~\eqref{eq:kappa-c}. Since
$p_{\mathrm{Cont},t\mid\theta_K}=\exp(-\Lambda_t\Delta t)\cdot
\exp(-\tilde\lambda_4(t)\Delta t)$ and the second factor is constant in
$\gamma_t^\ast$,
\begin{equation}
\label{eq:dpCont-dgamma}
\frac{\partial\,p_{\mathrm{Cont},t\mid\theta_K}}{\partial\gamma_t^\ast}
=\exp(-\tilde\lambda_4(t)\Delta t)\cdot
\frac{\partial}{\partial\gamma_t^\ast}\exp(-\Lambda_t\Delta t)
=-\kappa_h(\theta_K,t)\;p_{\mathrm{Cont},t\mid\theta_K}\;\Delta t.
\end{equation}

\paragraph{Step 2. Propagation to accumulated survival.}
Fix $t'\ge t$. Because only the factor $s=t$ in
$S(t'\mid\theta_K)=\prod_{s=1}^{t'}p_{\mathrm{Cont},s\mid\theta_K}$ depends on
$\gamma_t^\ast$,
\[
\frac{\partial S(t'\mid\theta_K)}{\partial\gamma_t^\ast}
=\Bigl(\prod_{\substack{s=1\\s\neq t}}^{t'}p_{\mathrm{Cont},s\mid\theta_K}\Bigr)
\frac{\partial\,p_{\mathrm{Cont},t\mid\theta_K}}{\partial\gamma_t^\ast}
=-\kappa_h(\theta_K,t)\;S(t'\mid\theta_K)\;\Delta t.
\]
For $t'<t$, the derivative is zero.

\paragraph{Step 3. Expected duration and sign.}
Differentiating~\eqref{eq:duracion-identidad-prop} term by term under~\textup{(S1)}
and applying Step~2 yields~\eqref{eq:dE-dgamma-prop}. The interchange of sum and
derivative is justified by dominated convergence: for $t'\ge t$,
$|\partial S(t'\mid\theta_K)/\partial\gamma_t^\ast|
\le|\kappa_h(\theta_K,t)|\,S(t'\mid\theta_K)\,\Delta t$, and
\[
\sum_{t'=t}^{\infty}S(t'\mid\theta_K)
\le\sum_{t'=0}^{\infty}S(t'\mid\theta_K)
=\mathbb{E}[\tau\mid\theta_K]<\infty
\]
by~\textup{(S0)}. Because $S(t'\mid\theta_K)\ge 0$ and the tail
$\sum_{t'=t}^{\infty}S(t'\mid\theta_K)>0$ whenever continuation has positive
mass on $t'\ge t$, the sign of~\eqref{eq:dE-dgamma-prop} is the opposite of the
sign of $\kappa_h(\theta_K,t)$.
\end{proof}

\begin{remark}[Local comparative static and testable content]
\label{rem:captivity-local-cs}
Proposition~\ref{prop:expected-captivity-gamma} is a \emph{local} comparative
static: it evaluates a one-period perturbation of $\gamma_t^\ast$ holding the
remaining policy path fixed. It does not claim global monotonicity of
$\mathbb{E}[\tau\mid\theta_K]$ as a function of the entire sequence
$\{\gamma_s^\ast\}_{s\ge 0}$. The empirical content is the sign prediction:
types with larger $\kappa_h$ respond differently to operational pressure because
the Cox coefficients $\beta_{K,j}(\theta_K)$ shift the semi-elasticities
$\{\eta_{\gamma,j}\}$ and the levels $\{\tilde\lambda_j\}$.
\end{remark}
...
\begin{definition}[Assessment and Perfect Bayesian Equilibrium]
...
$\mu_0(\theta)>0$ for all $\theta$. An \textbf{assessment} in this dynamic game is described by:
\begin{itemize}
...
Consider the PBE of Definition~\ref{def:pbe-mechanism} under the
...
By~(ii), the behavioral strategy space $\Sigma^i$ of each agent defined in the assessment (Definition~\ref{def:pbe-mechanism}) is a finite
product of simplices, compact and convex. By~(v), the feasible set of policies $\Gamma_t(\mu_t)$ defined in Definition~\ref{def:implementable-mechanism}
is compact, non-empty, with convex sections and continuous
correspondence. The state's objective in the sequential rationality condition~(a) of Definition~\ref{def:pbe-mechanism} is continuous in $(\alpha_t,\gamma_t)$
...
utilities of $K$ and $F$ from Definition~\ref{def:pbe-mechanism} are multilinear in behavioral strategies, so the best
...
At the fixed point, the state's strategy solves at each node on the path, satisfying the sequential rationality condition~(a) of Definition~\ref{def:pbe-mechanism}:
\[
...
-\psi_H\Delta H_t(\alpha_t,\gamma_t)\Bigr\}.
\]
The objective is continuous by~(iv) and the feasible set $\Gamma_t(\mu_t^\ast)$ (Definition~\ref{def:implementable-mechanism}) is compact and
...
By construction in Step~3, the equilibrium instruments satisfy $(\alpha_t^\ast,\gamma_t^\ast)\in\Gamma_t(\mu_t^\ast)$,
which by Definition~\ref{def:implementable-mechanism} simultaneously guarantees that the participation constraints $IR^K$ and $IR^F$ are satisfied in the
equilibrium profile.

Having completed the five steps, $(s^\ast,\mu^\ast)$ is a PBE (Definition~\ref{def:pbe-mechanism}) that implements
the social choice rule $f_t$ and is implementable in the sense of Definition~\ref{def:implementable-mechanism} (satisfying $IC^K$, $IR^K$, and $IR^F$ simultaneously).
\end{proof}
...
\citep{BlackwellDubins1962,Schervish1995Theory}. By the Bayesian updating rule defined in Definition~\ref{def:beliefs-martingale},
\[
\mu_t(\theta)
...
\texttt{lean/} (main file \texttt{AppendixBProofs.lean}) that
...
\ref{sec:info-bayes}--\ref{sec:id-abstracta}. Each declaration is tagged with its TeX
label where applicable. A successful build (\texttt{lake build}) yields no
\texttt{sorry} obligations. Build instructions appear in Section~\ref{sec:repro}.
\end{sloppypar}
...
\begin{itemize}
  ...
  Proposition~\ref{prop:expected-captivity-gamma}: compact and signed forms of
  $\kappa_h$ (\texttt{kappa\_h\_signed}, \texttt{dLambda\_eq\_kappa});
  continuation factorization and~\eqref{eq:dpCont-dgamma}
  (\texttt{dContProb\_eq}); sign certificate
  of~\eqref{eq:dE-dgamma-prop} with explicit $\Delta t$ and survival tail
  (\texttt{prop\_expected\_captivity\_gamma},
  \texttt{prop\_expected\_captivity\_sgn}). Wald identity and DCT remain
  analytic.
...
Proposition~\ref{prop:expected-captivity-gamma}, eq.~\eqref{eq:kappa-c}
  (signed form)
  & \texttt{kappa\_h\_signed}, \texttt{dLambda\_eq\_kappa} & Proved \\
Proposition~\ref{prop:expected-captivity-gamma}, eq.~\eqref{eq:dpCont-dgamma}
  & \texttt{dContProb\_eq} & Proved \\
Proposition~\ref{prop:expected-captivity-gamma}, eq.~\eqref{eq:dE-dgamma-prop}
  (sign)
  & \texttt{prop\-\_expected\-\_captivity\-\_gamma},
    \texttt{prop\-\_expected\-\_captivity\-\_sgn} & Part. \\
...
\bottomrule
\end{tabular}
\tabnote\ \texttt{[Proved]}: machine-checked in Lean~4. \texttt{[Part.]}:
partial certificate (sign of~\eqref{eq:dE-dgamma-prop} given the analytic
chain). \texttt{[Hyp.]}: declared interface; proved analytically in this
supplement or cited literature.
\tabsource
\end{table}
...
\texttt{tex/Appendix\_B\_Proofs.tex}. Machine-checked algebraic steps are reproduced by
the Lean~4 project in \texttt{lean/}. Set the working directory to
\texttt{Appendix\_B\_Proofs/} and run \texttt{replicate.sh}. See \texttt{README.txt} at the
root of that folder for software requirements (Lean~4, elan,
\texttt{lean-toolchain}) and Lake; Mathlib is fetched automatically on first
build (network required; estimated first-build time 15--45 minutes).
\end{sloppypar}

\begin{enumerate}
\item \texttt{replicate.sh} at the package root runs \texttt{lake build} in
  \texttt{lean/} and writes a log to \texttt{lean/build.log}.
\item Main formal file: \texttt{lean/AppendixBProofs.lean}. Project metadata:
  \texttt{lakefile.lean}, \texttt{lean-toolchain}, \texttt{lake-manifest.json}.
...
\item (Optional) Compile \texttt{tex/Appendix\_B\_Proofs.pdf} from \texttt{tex/} with
...
\texttt{Appendix\_B\_Proofs/}; see \texttt{README.txt} for the full build workflow.
\end{sloppypar}

\bibliographystyle{econsoc_B}
