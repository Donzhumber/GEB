% !TEX root = Appendix_A_Micro.tex
% =========================================================================
% APPENDIX A -- MICROSTRUCTURE AND COMPETING HAZARDS IN COLOMBIA
% Detailed presentation of Section~4.1 results. Replication: appendix_A_micro/scripts/
% =========================================================================
...
summarized in Section~4.1 of the main paper (``Empirical anchoring of the
parameter space''). It documents the CNMH/SIEVCAC data, the construction
...
\texttt{appendix\_A\_micro/scripts/} replication package (Section~\ref{sec:repro}).
...
(\texttt{Kidnap\_Type} $=$ \texttt{``EXTORSIVO''} in the source data) with a valid event date. The second
filter requires an observable liberation outcome (excluding not-observed,
other, and non-applicable codes in \texttt{Release\_Type}). The third
...
\bottomrule
\end{tabular}
\tabnote\ Valid event date: non-missing \texttt{Event\_Date} and \texttt{Year}$\neq 0$
...
The dependent variable \texttt{Y\_Outcome} classifies each extortionate
episode into four mutually exclusive outcomes (reference category: Payment):
\begin{itemize}
\item \textbf{Payment} ($Y=1$): ransom paid, hostage exchange, or
  intermediated payment.
\item \textbf{Rescue} ($Y=3$): liberation by state security forces.
\item \textbf{Escape or release without payment} ($Y=0$):
  flight or release under external pressure without ransom.
\item \textbf{Death} ($Y=2$): victim killed or found dead in captivity.
\end{itemize}
...
\input{tables_mnl_full}

\subsection{Key relative risk ratios}

Table~\ref{tab:mnl-key} summarizes the three headline findings in Section~4.1
...
Table~\ref{tab:robust} reports the key coefficient from the Death equation.

\begin{table}[t]
\centering
\caption{MNL Robustness: Death-Equation Coefficient for Perpetrator Group}
\label{tab:robust}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
robust standard errors in Robust SE. The log-relative-risk for common
delinquency is identical across all three specifications ($\hat\beta=4.15$,
$p<0.01$), confirming that the perpetrator-group effect in the Death equation
is not an artifact of the variance estimator or the inclusion of duration
categories.
\tabsource
\end{table}
...
\texttt{Days\_Captivity} (days in captivity). In each model, one outcome
...
lists the coefficients emphasized in Section~4.1 of the main paper.

\input{tables_cox_full}
...
Estimated on the Cox subsample ($N=461$) which drops 602 cases with missing duration
and 64 zero-day captivities. Reference perpetrator group: FARC. HR $=\exp(\tilde\beta)$;
HR $>1$ indicates faster resolution, HR $<1$ slower resolution. The Significance column
reports the highest conventional level (1\%, 5\%, 10\%); ``---'' denotes not
...
estimated on the Cox subsample ($N=461$). Duration categories: express
($\le 2$~days), short ($\le 30$~days), long ($>30$~days).
\tabsource
\end{table}
...
$t=98$ (maximum grid point in \path{stcurve} with \texttt{range(0 100)})
estimated on the Cox subsample ($N=461$) in \path{outputs/cumul_rescue_d100.dta},
exported to \path{outputs/Datos_Graficas_Cox.xlsx} by \path{microstructure_replication.do}.
\tabsource
\end{table}
...
corresponding cause-specific Cox margins reported in Section~4.1 of the main
...
(Section~4.1 of the main paper, ``Empirical anchoring of the parameter
space''). It introduces no new estimates. Every empirical quantity cited
...
MDG process. All paths are relative to \path{appendix_A_micro/scripts/}.
\tabsource
\end{table}
...
\begin{sloppypar}
All numerical results in this supplement are reproduced by Stata~19 scripts in
the \texttt{appendix\_A\_micro/scripts/} replication package. Run \path{run_all.do}
from the \path{appendix\_A\_micro/scripts/} directory. See \path{README.md}
for step-by-step instructions.
Estimated runtime: 5--15 minutes. Batch mode:
\texttt{stata -b do run\_all.do}.
\end{sloppypar}

\begin{sloppypar}
\begin{enumerate}
\item \path{run_all.do} runs \path{microstructure_replication.do},
  \path{sample_selection_robustness.do}, and \path{cox_only.do}
  in sequence; all output is saved to \path{outputs/replication.log}.
\item Each script includes \path{_setup_paths.do} and locates
  \path{data/Data_merge.dta} automatically.
\item Key outputs written to \path{outputs/} include
  \path{analytical_sample.dta}; \path{cox_appendix_rows.txt}
  (Table~\ref{tab:cox-hr}); \path{cox_cumul_day100.txt}
  (Table~\ref{tab:cox-cumul}); four \path{margins_*.dta} files
  (by zone, group, duration, profile); and \path{replication.log}
  (all model estimates and diagnostics).
\item \path{microstructure_replication.do} also exports
  \path{cumul_rescue_d100.dta} and \path{Datos_Graficas_Cox.xlsx}
  (Table~\ref{tab:cox-cumul}); GAULA margins in Table~\ref{tab:cox-gaula}
  are recorded in \path{replication.log}.
\item \path{tables_mnl_full.tex} and \path{tables_cox_full.tex}
  (Tables~\ref{tab:mnl-full} and~\ref{tab:cox-full}) are pre-formatted
  \LaTeX\ files; their entries are verified against \path{replication.log}.
\item (Optional) Compile the PDF: run \texttt{pdflatex}
  on \path{Appendix_A_Micro.tex} from the \path{appendix_A_micro/} directory.
\end{enumerate}
\end{sloppypar}
...
\path{data/Data_merge.dta} are documented in \path{README.md}.
\end{sloppypar}
