...
\title{Dynamic Mechanism Design with Active Learning and Endogenous Noise: Identifying Rational Types in Unknown Environments\tnoteref{t1}}
...
\begin{abstract}
This paper studies a dynamic mechanism design problem in which a planner, the principal, must intertemporally screen an adversary, the agent, whose equilibrium actions pool, so passive observation cannot separate types. The paper introduces the Mano de Dios--Guadalupe or MDG process, an endogenous-noise implementation law preserving full support off the equilibrium path without an exogenous trembling-hand perturbation. Combining this layer with a competing-risks structure and an active information-acquisition motive, the paper establishes the Type Identification Theorem, showing that uniform total-variation separation between each type's observation law forces mutual singularity of trajectory laws via the criterion of \citet{Kakutani1948}, so beliefs converge almost surely to the agent's true type. This identification result is complemented by a Conditional Mechanism Implementability Theorem guaranteeing a perfect Bayesian equilibrium that satisfies incentive compatibility and individual rationality whenever the feasible policy set is non-empty. Both results are machine-checked in Lean~4. Operational viability is illustrated with a calibration to kidnapping for ransom in Colombia, presented as applied validation.
\end{abstract}

\begin{keyword}
mechanism design \sep dynamic screening \sep Bayesian learning \sep active learning \sep identification \\
...
The third contribution is the Type Identification Theorem for Rational Types in Unknown Environments. The result establishes that if the structural fingerprint $\beta_{K,j}(\theta_K)$ of each type induces uniform separation in total variation between daily observation laws, then the criterion of \citet{Kakutani1948} for products of probability measures implies mutual singularity of trajectory laws and, in the prolonged-captivity event $\mathcal{T}=\{\tau=\infty\}$, the vector of Bayesian beliefs $\mu_t$ converges almost surely to the Dirac measure over the true type, $\Dirac_{\theta_K^\ast}$, in accordance with the axioms of \citet{BlackwellDubins1962}. The proof starts from the asymptotic stability of a bounded martingale in the sense of \citet{Doob1953Stochastic} and appeals to the criterion of \citet{Kakutani1948} to close identification. The theorem converts a local daily separation condition into global posterior identification conditional on $\mathcal{T}$. The Conditional Mechanism Implementability Theorem in PBE complements this result: under the hypothesis that the feasible set $\Gamma_t(\mu_t)$ remains non-empty, there exists a PBE in which the state's optimal rule simultaneously satisfies $IC^K$, $IR^K$, and $IR^F$.

The rest of the article proceeds as follows. Section~\ref{sec:institutional} translates the Colombian evidence into constraints on types and costs; Section~\ref{sec:pertinencia} shows how the empirical models anchor the prior of the mechanism; Section~\ref{sec:model} develops the dynamic model, the state's program, and the identification theorem; and Section~\ref{sec:calibracion} presents the calibration and the ELN--PAR simulations. The Supplemental Appendices contain detailed macroeconometric results, microstructure and competing hazards, and formal proofs with Lean~4 verification.


\section{Empirical Anchoring of the Parameter Space}\label{sec:institutional}

Between 1964 and 2025, Colombia registered 61,341 kidnapping cases \citep{cnmh_datos}, concentrated in specific municipalities and closely tied to the evolution of the internal armed conflict; the 1996 creation of the Unified Action Groups for Personal Liberty (GAULA, \citealt{ley282_1996}) marked the state's institutional response to this dynamic, and the resulting variation in coercive capacity, legal reform, and organizational structure across armed groups is precisely the source of the type heterogeneity the mechanism is designed to screen. This applied motivation is developed at length, together with the underlying macroeconometric analysis (a Vector Error Correction model and a Local Linear Trend model of kidnapping's stochastic trend, formally reconciled by the trend-equivalence corollary), in Appendix~A; the full microeconometric estimation underlying the table below is presented in Appendix~B.

The parameters that feed the mechanism's state space and technologies are obtained from a Multinomial Logit (MNL) model of case outcomes and a cause-specific proportional hazards model of \citet{Cox1972}, estimated on individual-level microdata from the National Center for Historical Memory \citep{cnmh_datos} ($n=1{,}125$ analytical sample; full specification, robustness checks, and sample-selection diagnostics in Appendix~B). Table~\ref{tab:hazard-prior-summary} consolidates the resulting per-type technology fingerprint for the four captor types $\ThetaK=\{\text{DC},\text{PAR},\text{ELN},\text{FARC}\}$.

\begin{table}[H]
\centering
\caption{Consolidated per-type hazard and outcome fingerprint (DC, PAR, ELN, FARC)}
\label{tab:hazard-prior-summary}
\begin{tabular}{lccc}
\toprule
Type $\theta_K$ & Mortality margin (MNL) & Payment hazard ratio (Cox, ref.\ FARC) & Cum.\ Rescue @ day 100 (Cox) \\
\midrule
FARC & 0.002 & 1.00 (reference) & 0.27 \\
ELN  & 0.007 & $\approx 1.00$ (n.s., $p=0.317$) & 0.28 \\
PAR  & 0.026 & $4.28$ ($p<0.001$) & 0.33 \\
DC   & 0.071 & $1.71$ ($p=0.021$) & n.r.\ (see Appendix~B) \\
\bottomrule
\end{tabular}
\end{table}

The mapping from these estimates to the theoretical model of Section~\ref{sec:model} is direct. The mortality margins order the reservation utility of release $U^K_{\mathrm{rel}}(\theta)=-\kappa_{\mathrm{rel}}(\theta)$ across types, since organizations with lower logistical retention capacity (higher mortality) face a correspondingly different calculus in the $IR^K$ participation constraint. The Cox hazard ratios are transferred to the mechanism as the type-specific effective intensities $\tilde\lambda_j(t\mid\theta_K)$ that govern the daily competing-risks signal $\zeta_t$ and, through it, the Bayesian update of $\mu_t$; their empirical separation across types is the observational counterpart of the uniform total-variation separation hypothesis in Lemma~\ref{lem:hazard-separation}, on which Theorem~\ref{thm:posterior-consistency} rests. The scenario-specific initial beliefs $\mu_0(\theta\mid z,\theta_V)$ used in the calibration of Section~\ref{sec:calibracion} are reported there together with each simulated scenario.

\section{Relevance of the Empirical Framework for the Mechanism Model}\label{sec:pertinencia}

The theoretical model in Section~\ref{sec:model} requires two empirical inputs so as not to depend on ad hoc assumptions: a persistent macroeconomic environment that disciplines retention costs and a type structure with differentiated priors, technologies, and hazards. The VEC system, the Gonzalo--Granger decomposition, and the trend-equivalence corollary (Appendix~A) provide the first input. The cointegration among $K^E$, $K^{NE}$, and $r$ establishes that kidnapping for ransom does not evolve as an isolated series, but as part of a long-run system in which coercive capacity and macro-financial conditions act as permanent forces. The equivalence between the VEC--GG trend and the LLT trend prevents calibration from depending on a single econometric specification, while the adjustment structure of the VEC shows that $K^E$ is the margin that absorbs disequilibria. Therefore, the state vector of the mechanism, the feasible set $\Gamma_t(\mu_t)$, and the constraints $IC^K$ and $IR^{K,F}$ are anchored in persistent regularities, not in a purely theoretical normalization.
...
\subsubsection*{Proof Sketch and Mathematical Intuition}

The proof, given in full in Appendix~3, assembles three ingredients already introduced above; the logic connecting them is sketched here so the result is legible without leaving the main text.

\emph{(1) Persistent technological fingerprints separate daily signal laws.} The cause-specific hazards $\tilde\lambda_j(t\mid\theta_K)$ that govern the daily competing-risks signal $\zeta_t$ depend on $\theta_K$ only through the coefficient vector $\beta_{K,j}(\theta_K)$ (Section~\ref{sec:model}); whenever two types $\theta,\theta'$ have $\beta_{K,j}(\theta)\neq\beta_{K,j}(\theta')$ for some cause $j$, the induced one-period outcome distributions differ pointwise, which by Lemma~\ref{lem:hazard-separation} part~\textup{(i)} yields a strictly positive total-variation gap $\mathrm{TV}(p_\theta^{(t)},p_{\theta'}^{(t)})\ge\varepsilon>0$ in every period in which the technological asymmetry is active. Because $\beta_{K,j}(\theta_K)$ is a structural, time-invariant parameter of the type rather than an artifact of a particular period's instruments, this local separation recurs along an infinite subset of periods whenever the episode remains open (part~\textup{(ii)}), rather than vanishing after finitely many observations.

\emph{(2) The entropy-governed MDG temperature does not erase the signal.} The executed action $\tilde a_t^i$ is a noisy implementation of the latent intention $a_t^{i\ast}$ through the hybrid-temperature logit of the MDG process, with temperature $T_t=T_0\max\{H(\mu_t)/H(\mu_0)\,e^{-\eta_{\mathrm{cal}}t},\underline{c}\}$ bounded below by the floor $\underline c>0$. Because $T_t$ is bounded away from zero and from infinity for every finite $t$, the MDG noise guarantees full support (Axiom A1) without ever assigning the competing-risks signal a degenerate, fully deterministic law; the noise dilutes but does not destroy the total-variation gap established in step~(1), so the separation margin $\varepsilon_\theta$ in Lemma~\ref{lem:hazard-separation} remains strictly positive even after passing through the implementation layer.

\emph{(3) Martingale convergence and the Kakutani criterion close identification.} By Definition~\ref{def:beliefs-martingale}, $\mu_t(\theta)$ is, for each fixed $\theta$, a bounded martingale with respect to the public filtration $(\mathcal F_t^{\mathrm{pub}})_{t\ge0}$; Doob's martingale convergence theorem (\citealt{Doob1953Stochastic}) then guarantees an almost-sure limit $\mu_\infty(\theta)\in[0,1]$ (Lemma~\ref{lem:asymptotic-beliefs}). This limit argument alone does not pin down \emph{where} the mass concentrates. That is supplied by the recurrent total-variation separation of steps~(1)--(2): under condition~(C3), infinitely many periods with $\mathrm{TV}\ge\underline\varepsilon_\theta>0$ force the Hellinger affinity between the trajectory laws of $\theta$ and $\theta^\ast$, $\rho_t=\sum_y\sqrt{p_\theta^{(t)}(y)\,p_{\theta^\ast}^{(t)}(y)}\le 1-\underline\varepsilon_\theta^2/2<1$ in each such period, to vanish in the infinite product $\prod_t\rho_t\to0$; the criterion of \citet{Kakutani1948} for infinite products of probability measures then converts this vanishing product of affinities into mutual singularity of the trajectory laws $\ProbC\theta(\cdot\mid\mathcal T)$ and $\ProbC{\theta^\ast}(\cdot\mid\mathcal T)$. Mutual singularity, together with the martingale limit of step~(3), forces the false-type posterior mass to collapse to zero ($\mu_t(\theta)\to0$), and simplex normalization on the finite type space $\Delta(\ThetaK)$ then delivers $\mu_t(\theta^\ast)\to1$.

\paragraph{Lean~4 formalization.} The measure-theoretic core of this argument is machine-checked in a standalone Lean~4 companion file (\texttt{Appendix3Proofs.lean}, distributed with the supplement), built on \texttt{Mathlib}'s probability and martingale libraries (\texttt{Mathlib.Probability.Martingale.Basic}, \texttt{Mathlib.Probability.Martingale.Convergence}, \texttt{Mathlib.Probability.Process.Filtration}, and the conditional-expectation and $L^p$-seminorm modules of \texttt{Mathlib.MeasureTheory}). The file mirrors the eight sections of Appendix~3 and marks every result as either \texttt{[Proved]} (machine-checked) or \texttt{[Hypothesis]} (a structural or classical assumption stated as an interface, not discharged in Lean). The martingale step is fully machine-checked: boundedness, the tower property, and almost-sure convergence to a $[0,1]$-valued limit (Lemma~\ref{lem:asymptotic-beliefs}) are proved from Mathlib's submartingale convergence theorem without additional hypotheses. The competing-risks algebra of step~(1) is likewise fully proved: that $\mathrm{predictiveMass}$ defines a genuine probability mass, and that distinct Cox coefficients $\beta_{K,j}(\theta)\neq\beta_{K,j}(\theta')$ force a strictly positive total-variation margin, are machine-checked theorems, not assumptions. Two components of the argument are recorded as explicit \texttt{[Hypothesis]} interfaces rather than fully formalized: the maximum-entropy characterization of the MDG logit (Axiom~A3, which requires the variational calculus of \citealt{Jaynes1957Information} that is not part of Mathlib), and the classical measure-theoretic step of Kakutani's criterion itself (\texttt{kakutaniMutualSingularity}), which is stated as a hypothesis rather than reproved from first principles inside Lean. The formalization therefore certifies the novel probabilistic bridge specific to this mechanism, the recurrent Cox separation implying asymptotic identification, while appropriately treating two classical results from the mathematics literature as imported facts rather than re-deriving them.
...
\smallskip
\noindent PBE. The pair $(s,\mu)$ is a PBE if conditions \textup{(a)} and \textup{(b)} hold:
\begin{enumerate}
...
\end{equation}
Building on the existence framework of \citet{KadanRenySwinkels2017}, Theorem~\ref{thm:ebp-implementabilidad} treats the non-emptiness of $\Gamma_t(\mu_t)$ as the primitive robust-feasibility condition ensuring the existence of an implementable PBE. The proof is presented in Appendix~3.
\end{definition}
...
The macroeconomic cointegration results, the competing-risks microeconometric hazards, and the calibrated Bayesian dual-control mechanism jointly support the following guidelines for public policy.

\textbf{Integration of macro-financial alerts into operational security planning.}\quad
...
\textbf{Adaptive and coordinated implementation of the financial block to avoid collusion.}\quad
...
\textbf{Transition to a police intelligence protocol based on Bayesian dual control.}\quad
