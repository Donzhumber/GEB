...
\title{Identificación de Tipos Racionales en Entornos Desconocidos bajo un Mecanismo Dinámico Indirecto\tnoteref{t1}}
...
\begin{abstract}
Este artículo desarrolla un mecanismo dinámico indirecto para la identificación de tipos racionales en entornos desconocidos caracterizados por agrupamiento observacional. La contribución teórica central introduce el proceso Mano de Dios--Guadalupe (MDG), un núcleo de implementación con ruido endógeno que mapea intenciones latentes sobre acciones ejecutadas con soporte completo dentro del juego bayesiano. Al integrar el proceso MDG con un marco de riesgos competidores, se establece el Teorema de Identificación de Tipos Racionales, en el cual las creencias bayesianas posteriores convergen casi seguramente hacia el tipo verdadero. Asimismo, se presenta el Teorema de Implementabilidad, que garantiza que el mecanismo es ejecutable en un equilibrio bayesiano perfecto. La aplicación empírica al secuestro extorsivo en Colombia valida el marco teórico mediante una calibración microeconométrica y simulaciones estructurales que demuestran la robustez del aprendizaje bayesiano y del cumplimiento de las restricciones de incentivos.
\end{abstract}

\begin{keyword}
diseño de mecanismos indirectos \sep selección dinámica \sep ruido de implementación endógeno \sep aprendizaje bayesiano \sep identificación de tipos \\
...
El horizonte del episodio no está fijado de antemano sino que está determinado por la propia dinámica del cautiverio, en el cual el episodio concluye en el primer período en el que se materializa un desenlace terminal, de modo que el horizonte efectivo es el tiempo de parada endógeno.
\begin{equation}
...
\begin{equation}
a_t^{K\ast}=\arg\max_{a_t^K\in\mathcal{A}^K}\mathcal{U}^K_t(a_t^K\mid h_t,\theta_K,\theta_S),\quad
...
\end{equation}
Apoyándose en el marco de existencia de \citet{KadanRenySwinkels2017}, el Teorema~\ref{thm:ebp-implementabilidad} trata la no vacuidad de $\Gamma_t(\mu_t)$ como la condición primitiva de factibilidad robusta que garantiza la existencia de un EBP implementable. La demostración se presenta en el Apéndice~B.
\end{definition}
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \parbox[t][\subfigheight][b]{\linewidth}{%
    \centering\includegraphics[width=\linewidth,height=\subfigheight,keepaspectratio]{Fig_9a.pdf}%
  }
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \parbox[t][\subfigheight][b]{\linewidth}{%
    \centering\includegraphics[width=\linewidth,height=\subfigheight,keepaspectratio]{Fig_9b.pdf}%
  }
...
El MNL y el modelo de Cox conectan la evidencia microeconómica con la distribución inicial $\mu_0\in\Delta(\ThetaK)$ y los vectores de tecnología $\beta_{K,j}(\theta_K)$. El MNL provee la huella estática de cada tipo al relacionar perpetrador, víctima, contexto y tácticas con la distribución condicional de desenlaces; así, la creencia inicial y los pagos terminales se fijan con pesos observados. El modelo de Cox provee la dimensión dinámica: sus hazards específicos por causa se trasladan al mecanismo como tasas $\lambda_j(\theta_K,t)$ que gobiernan la señal diaria $\zeta_t$, actualizan el posterior $\mu_t$ y determinan la velocidad del aprendizaje. La separación empírica de esas tasas entre organizaciones es la base observacional del Lema~\ref{lem:hazard-separation} y del criterio de \citet{Kakutani1948}: si las trayectorias inducidas por los tipos generan leyes distintas, el posterior puede concentrarse en el tipo verdadero. En conjunto, este bloque microeconométrico convierte los datos a nivel de caso en priors, costos, hazards y restricciones directamente utilizables por el mecanismo dinámico. El protocolo explícito de traducción entre estimadores y parámetros estructurales se detalla al comienzo de la Sección~\ref{sec:calibracion}.

\section{Calibración del mecanismo y simulaciones}\label{sec:calibracion}
\normalfont\normalsize
Esta sección calibra el mecanismo especializado de la Sección~\ref{sec:application} en cuatro episodios simulados de horizonte finito, manteniendo fija la arquitectura general de la Sección~\ref{sec:model}. El ejercicio combina priors y diagnósticos microeconométricos para estudiar cuatro objetos, que corresponden a la evolución de las creencias $\mu_t(\theta)$, las señales observadas por el Estado, la respuesta de los instrumentos $(\alpha_t,\gamma_t)$, y el cumplimiento de las restricciones IR e IC. Establecer estos cuatro objetos requiere resolver el mecanismo completo mediante inducción hacia atrás, y por ello se recurre a la inteligencia artificial, dado que la resolución de la ecuación de Bellman para la rama de continuación del secuestrador mediante optimización tradicional resulta computacionalmente costosa, pues exige recalcular recursivamente el valor esperado de mantener el cautiverio en cada período; este mismo costo recursivo se propaga a la solución de todo el mecanismo, ya que el problema de control dual del Estado necesita ese valor de continuación en cada período para determinar los instrumentos óptimos que garanticen $\Gamma_t^{a^S}(\mu_t)$. Para reducir drásticamente este costo computacional y anticipar las decisiones futuras, el modelo emplea un aproximador de valor basado en un perceptrón multicapa (MLP), entrenado en dos instancias independientes, donde una aproxima el valor de continuación que alimenta la optimización del Estado, y la otra aproxima la respuesta óptima del secuestrador bajo su tipo verdadero. Esta aproximación mediante redes neuronales funciona como un simulador avanzado que analiza 34 variables del entorno para predecir qué valor le asigna cada actor a continuar la interacción. La red se entrenó mediante algoritmos de aprendizaje profundo que evaluaron 2.000 escenarios posibles generados por computadora. En la práctica, el sistema realiza este cálculo en tiempo real para determinar de forma óptima tanto la respuesta del captor como la política del Estado ante distintos niveles de presión e instrumentos de control. Todos los detalles matemáticos de la arquitectura, la configuración de la red, y el proceso de calibración computacional se desarrollan completamente en el Apéndice C.

Para evaluar el desempeño dinámico del modelo, la calibración se inicializa mediante la selección de un conjunto de parámetros iniciales y covariables que definen el entorno del secuestro. En este diseño, el modelador determina de forma exógena el tipo privado del captor, lo cual constituye el benchmark de verdad para medir el aprendizaje. No obstante, en consistencia con el supuesto de información incompleta, el mecanismo dinámico opera de tal forma que tanto el Estado como la familia desconocen dicho tipo privado, interactuando y decidiendo sus estrategias óptimas únicamente a partir de sus creencias o priors bayesianos.

El \emph{Escenario~I} corresponde a DC, con $\theta_K^\ast=\mathrm{DC}$, Estado laxo con la presión sobre el crimen, familia de alta riqueza, víctima privada, y localización Metrópoli; el \emph{Escenario~II} corresponde a PAR, con $\theta_K^\ast=\mathrm{PAR}$, Estado estricto, familia de riqueza estándar, víctima pública, y localización Caribe; el \emph{Escenario~III} corresponde a ELN, con $\theta_K^\ast=\mathrm{ELN}$, Estado estricto, familia de alta riqueza, víctima privada, y localización Oriente-Selva; y el \emph{Escenario~IV} corresponde a FARC, con $\theta_K^\ast=\mathrm{FARC}$, Estado laxo con la presión sobre el crimen, familia de alta riqueza, víctima pública, y localización Pacífico / Zona Roja. Los cuatro se simulan sobre un horizonte fijo $\bar{T}=150$ períodos (días), y la política óptima se calcula a partir de la minimización del costo social del Estado. En el orden $(\mathrm{DC},\mathrm{PAR},\mathrm{ELN},\mathrm{FARC})$, los priors iniciales son $\mu_0^{\mathrm{DC}}=(0.35,0.35,0.15,0.15)$, $\mu_0^{\mathrm{PAR}}=(0.15,0.15,0.25,0.45)$, $\mu_0^{\mathrm{ELN}}=(0.05,0.35,0.25,0.35)$, y $\mu_0^{\mathrm{FARC}}=(0.35,0.35,0.15,0.15)$. Las Tablas~\ref{tab:covariables-escenarios} y \ref{tab:priors-escenarios} presentan el resumen estructurado de los cuatro escenarios evaluados, detallando respectivamente la configuración de las covariables contextuales de Estado, familia, víctima y localización, así como las creencias iniciales (\emph{priors}) asignadas a cada tipo de captor.

\begin{table}[!ht]
\centering
\small
\caption{Parámetros y covariables por escenario de calibración}
\label{tab:covariables-escenarios}
\begin{tabular}{lcccc}
\toprule
Escenario & Estado & Familia & Víctima & Localización \\ \midrule
I: DC & Laxo & Alta riqueza & Privada & Metrópoli \\
II: PAR & Estricto & Estándar & Pública & Caribe \\
III: ELN & Estricto & Alta riqueza & Privada & Oriente-Selva \\
IV: FARC & Laxo & Alta riqueza & Pública & Pacífico / Zona Roja \\ \bottomrule
\end{tabular}
\par\smallskip\footnotesize \textit{Fuente:} Elaboración propia.

\vspace{24pt}

\caption{Creencias iniciales (\emph{priors}) por escenario de calibración}
\label{tab:priors-escenarios}
\begin{tabular}{lcccc}
\toprule
Escenario & $\mu_0(\mathrm{DC})$ & $\mu_0(\mathrm{PAR})$ & $\mu_0(\mathrm{ELN})$ & $\mu_0(\mathrm{FARC})$ \\ \midrule
I: DC & 0.35 & 0.35 & 0.15 & 0.15 \\
II: PAR & 0.15 & 0.15 & 0.25 & 0.45 \\
III: ELN & 0.05 & 0.35 & 0.25 & 0.35 \\
IV: FARC & 0.35 & 0.35 & 0.15 & 0.15 \\ \bottomrule
\end{tabular}
\par\smallskip\footnotesize \textit{Fuente:} Elaboración propia.
\end{table}

El posterior del tipo verdadero permite evaluar la capacidad del mecanismo para separar tipos a partir de la secuencia de señales. Como se reporta en la Figura~\ref{fig:cal-eln-par-mu}, la masa posterior converge gradualmente hacia el tipo verdadero en los cuatro escenarios, alcanzando $\mu_{150}(\theta_K^\ast)\approx 1{,}000$; esto implica que el mecanismo acumula identificación bajo diferentes niveles de riqueza, regiones y visibilidad. Las trayectorias reflejan el aprendizaje bayesiano dinámico a partir de las señales observadas en cada ciclo, hasta que la masa de creencia se concentra definitivamente en el verdadero perpetrador. Las barras grises marcan el tiempo de parada realizado; la extensión contrafactual subsiguiente muestra que la velocidad de aprendizaje y el momento de absorción son objetos distintos del modelo. En conjunto, la figura ilustra el comportamiento a horizonte finito: la masa posterior se desplaza hacia el tipo verdadero a medida que las señales acumulan separación, en la dirección predicha por el Teorema~\ref{thm:posterior-consistency} bajo $\mathcal{T}$, sin constituir evidencia directa del resultado asintótico; el aprendizaje se consolida bajo el entorno institucional, territorial, y de visibilidad de cada caso.
...
\end{subfigure}
\caption{Creencias bayesianas de cada tipo $\mu_\tau(\theta)$ en los diferentes escenarios (en color en la versión digital).}
\label{fig:cal-eln-par-mu}
\figurenote{\textit{Nota:} El panel (a) corresponde al Escenario~I: $\theta_K^\ast=\mathrm{DC}$, $\theta_S$ laxo, $\theta_F$ de alta riqueza, $\theta_V$ privado, y $z$ Metrópoli. El panel (b) corresponde al Escenario~II: $\theta_K^\ast=\mathrm{PAR}$, $\theta_S$ estricto, $\theta_F$ de riqueza estándar, $\theta_V$ público, y $z$ Caribe. El panel (c) corresponde al Escenario~III: $\theta_K^\ast=\mathrm{ELN}$, $\theta_S$ estricto, $\theta_F$ de alta riqueza, $\theta_V$ privado, y $z$ Oriente-Selva. El panel (d) corresponde al Escenario~IV: $\theta_K^\ast=\mathrm{FARC}$, $\theta_S$ laxo, $\theta_F$ de alta riqueza, $\theta_V$ público, y $z$ Pacífico / Zona Roja. La banda gris marca $\tau^{\mathrm{hyp}}$, el primer período en el que el resultado del mecanismo difiere de la continuación. \textit{Fuente:} Elaboración propia.}
\end{figure}

La presión operativa óptima permite evaluar cómo ajusta el Estado la intensidad de la intervención a medida que aprende sobre el tipo del captor. La Figura~\ref{fig:cal-eln-par-gamma} compara la trayectoria de $\gamma_t^\ast$ con los puntos de referencia ponderados por creencias de las ramas de Rescate ($\gamma_R^\mu$) y Negociación ($\gamma_N^\mu$). En la línea base, los valores límite de rescate por tipo son $\gamma_R^{\mathrm{DC}}=0{,}23$, $\gamma_R^{\mathrm{PAR}}=0{,}37$, $\gamma_R^{\mathrm{ELN}}=0{,}50$ y $\gamma_R^{\mathrm{FARC}}=0{,}64$, mientras que los de negociación son $\gamma_N^{\mathrm{DC}}=0{,}59$, $\gamma_N^{\mathrm{PAR}}=0{,}64$, $\gamma_N^{\mathrm{ELN}}=0{,}69$ y $\gamma_N^{\mathrm{FARC}}=0{,}74$. En el Escenario~I (DC), $\gamma_t^\ast$ se ubica en promedio alrededor de $\bar\gamma^\ast\approx 0{,}564$. En el Escenario~II (PAR), la presión alcanza $\bar\gamma^\ast\approx 0{,}662$. En el Escenario~III (ELN), la presión promedio es $\bar\gamma^\ast\approx 0{,}663$, y en el Escenario~IV (FARC) alcanza $\bar\gamma^\ast\approx 0{,}715$. En todos los casos, $\gamma_t^\ast$ no replica de forma idéntica ninguna de las ramas puras desde el inicio; la política fluctúa entre ambos referentes bajo control dual según el riesgo material y el valor de separar tipos. No obstante, a medida que la creencia posterior $\mu_t$ converge hacia el tipo verdadero del captor, el instrumento óptimo converge de manera natural al benchmark de información completa correspondiente al verdadero perpetrador en la rama ejecutada, aproximándose aquí sistemáticamente a la referencia de Negociación.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario IV: FARC}
\end{subfigure}
\caption{Presión operativa $\gamma_t^\ast$ y puntos de referencia $\gamma_R^\mu$ y $\gamma_N^\mu$ frente a $\tau$.}
\label{fig:cal-eln-par-gamma}
\figurenote{\textit{Nota:} El panel (a) muestra la trayectoria de presión operativa en el Escenario~I (DC). El panel (b) muestra la trayectoria equivalente en el Escenario~II (PAR). El panel (c) muestra el Escenario~III (ELN). El panel (d) muestra el Escenario~IV (FARC). Las líneas punteadas corresponden a los puntos de referencia de Rescate ($\gamma_R^\mu$) y Negociación ($\gamma_N^\mu$) ponderados por creencias. La banda gris marca $\tau^{\mathrm{hyp}}$. \textit{Fuente:} Elaboración propia.}
\end{figure}

Al igual que en el caso de la presión operativa, la interdicción financiera óptima resume la intensidad con la que el Estado restringe la capacidad de pago y la coordinación económica durante el episodio. La Figura~\ref{fig:cal-eln-par-alpha} compara la trayectoria de $\alpha_t^\ast$ con los puntos de referencia ponderados por creencias de las ramas de Rescate ($\alpha_R^\mu$) y Negociación ($\alpha_N^\mu$). En la línea base, los valores límite de rescate por tipo son $\alpha_R^{\mathrm{DC}}=0{,}18$, $\alpha_R^{\mathrm{PAR}}=0{,}31$, $\alpha_R^{\mathrm{ELN}}=0{,}45$ y $\alpha_R^{\mathrm{FARC}}=0{,}59$, mientras que los de negociación son $\alpha_N^{\mathrm{DC}}=0{,}62$, $\alpha_N^{\mathrm{PAR}}=0{,}67$, $\alpha_N^{\mathrm{ELN}}=0{,}72$ y $\alpha_N^{\mathrm{FARC}}=0{,}77$. En el Escenario~I (DC), $\bar\alpha^\ast\approx 0{,}896$. En el Escenario~II (PAR), $\bar\alpha^\ast\approx 0{,}730$. En el Escenario~III (ELN), $\bar\alpha^\ast\approx 0{,}737$, y en el Escenario~IV (FARC), $\bar\alpha^\ast\approx 0{,}755$. En todos los casos, $\alpha_t^\ast$ no replica de forma idéntica ninguna de las ramas puras desde el inicio; la política fluctúa entre ambos referentes bajo control dual según la información acumulada, la capacidad de pago de la familia y el valor de inducir separación entre tipos. En escenarios de capacidad estatal estricta, la necesidad de separar tipos y evitar la colusión familiar puede situar temporalmente la interdicción por encima de ambos referentes. No obstante, a medida que la creencia posterior $\mu_t$ converge hacia el tipo verdadero del captor, el instrumento óptimo converge de manera natural al benchmark de información completa correspondiente al verdadero perpetrador en la rama ejecutada, aproximándose aquí sistemáticamente a la referencia de Negociación.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario IV: FARC}
\end{subfigure}
\caption{Interdicción financiera $\alpha_t^\ast$ y puntos de referencia $\alpha_R^\mu$ y $\alpha_N^\mu$ frente a $\tau$.}
\label{fig:cal-eln-par-alpha}
\figurenote{\textit{Note:} El panel (a) presenta la interdicción financiera óptima en el Escenario~I (DC). El panel (b) presenta el Escenario~II (PAR). El panel (c) presenta el Escenario~III (ELN). El panel (d) presenta el Escenario~IV (FARC). Las líneas punteadas corresponden a los puntos de referencia de Rescate ($\alpha_R^\mu$) y Negociación ($\alpha_N^\mu$) ponderados por creencias. La banda gris marca $\tau^{\mathrm{hyp}}$. \textit{Fuente:} Elaboración propia.}
\end{figure}
...
\figurenote{\textit{Nota:} El panel (a) corresponde al Escenario~I (DC), el panel (b) al Escenario~II (PAR), el panel (c) al Escenario~III (ELN), y el panel (d) al Escenario~IV (FARC). Cada trayectoria muestra 20 flechas indicadoras; el círculo identifica el punto final, $\tau=150$. \textit{Fuente:} Elaboración propia.}
\end{figure}
...
\figurenote{\textit{Note:} El panel (a) corresponde al Escenario~I (DC), el panel (b) al Escenario~II (PAR), el panel (c) al Escenario~III (ELN), y el panel (d) al Escenario~IV (FARC). Cada trayectoria muestra 20 flechas indicadoras; el círculo identifica el punto final, $\tau=150$. \textit{Fuente:} Elaboración propia.}
\end{figure}
...
\figurenote{\textit{Nota:} El panel (a) resume el cumplimiento en el Escenario~I (DC), el panel (b) en el Escenario~II (PAR), el panel (c) en el Escenario~III (ELN), y el panel (d) en el Escenario~IV (FARC). Los porcentajes se calculan sobre los ciclos simulados de cada escenario. \textit{Fuente:} Elaboración propia.}
\end{figure}

La ganancia informacional esperada $\Delta H_t$ cuantifica la reducción anticipada de entropía inducida por los instrumentos del Estado, en el sentido de la adquisición dinámica óptima de información de \citet{Zhong2022}. Ese subsidio informacional se concentra al inicio de la simulación, como se ilustra en la Figura~\ref{fig:cal-eln-par-dh}: en DC alcanza su máximo en $\tau=3$ con $\Delta H_3\approx 0{,}028$, y en PAR en $\tau=2$ con $\Delta H_2\approx 0{,}027$. A medida que $\Delta H_t$ converge a cero, las observaciones adicionales dejan de reducir la incertidumbre posterior y la decisión óptima pasa a estar gobernada por las pérdidas materiales y una creencia ya concentrada en el tipo verdadero.
...
\figurenote{\textit{Nota:} El panel (a) corresponde al Escenario~I (DC), el panel (b) al Escenario~II (PAR), el panel (c) al Escenario~III (ELN), y el panel (d) al Escenario~IV (FARC). La serie reporta la reducción esperada de entropía en cada ciclo simulado. \textit{Fuente:} Elaboración propia.}
\end{figure}

A diferencia de una relación uniforme, el signo de la sensibilidad neta $\kappa_h(\theta_K^\ast,t)$ varía entre escenarios, como se ilustra en la Figura~\ref{fig:cal-eln-par-kh}. En el Escenario~II (PAR) y el Escenario~IV (FARC), $\kappa_h > 0$ en la totalidad o la gran mayoría de los ciclos (100\,\% y 91{,}3\,\%, respectivamente), lo que implica $\partial\,\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast<0$: una mayor presión operativa acorta el cautiverio. En contraste, en el Escenario~I (DC) la sensibilidad es predominantemente negativa ($\kappa_h < 0$ en el 82{,}0\,\% de los períodos), lo que implica que una mayor presión prolonga el cautiverio al inhibir el canal de pago directo sin compensar suficientemente mediante rescates. En el Escenario~III (ELN), la trayectoria fluctúa entre ambos regímenes. El modelo ilustra así un trade-off estratégico: la presión acorta el cautiverio solo cuando el riesgo de prolongación por disuasión de negociación es dominado por la efectividad del rescate o la capitulación.
...
\figurenote{\textit{Nota:} Cada panel traza $\kappa_h(\theta_K^\ast,t)$ para el tipo verdadero del escenario. Un valor constante de $+1$ implica $-\operatorname{sgn}(\kappa_h)=-1$ en el 100\,\% de los ciclos, es decir, $\partial\,\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast<0$; la línea punteada horizontal marca el cero de referencia. \textit{Fuente:} Elaboración propia.}
\end{figure}
...
En conjunto, el contraste de los cuatro escenarios muestra que el mismo mecanismo bayesiano genera trayectorias de aprendizaje, intensidades de instrumentos, y tasas de viabilidad de restricciones distintas cuando, conjuntamente, cambian el tipo verdadero del captor y los márgenes institucional, familiar, de víctima, y geográfico descritos en el protocolo. Las simulaciones no sustituyen al Teorema~\ref{thm:posterior-consistency}, que requiere separación recurrente en el evento $\mathcal{T}$; ilustran que, con priors y hazards calibrados a la evidencia colombiana, el subsidio entrópico desplaza la política lejos de regímenes puramente miopes y acelera o disciplina la concentración posterior en horizontes finitos, en la dirección predicha por el resultado de identificación.

\section{Simulaciones bajo Exclusión de la Comunicación de Voz}\label{sec:no-voice}

Para evaluar la robustez y suficiencia del mecanismo de identificación, esta sección analiza las simulaciones dinámicas bajo la exclusión contrafactual del canal de comunicación de voz, diferenciándose de la Sección~\ref{sec:calibracion} al eliminar por completo la información acústica mediante la condición $V_t = 0$ para todo $t$. Mientras que en la calibración de referencia de la Sección~\ref{sec:calibracion} la actualización bayesiana del Estado incorpora tanto las señales físico-institucionales como las llamadas de voz, aquí aislamos los canales físico-institucionales endógenos desactivando la retroalimentación de verosimilitud de la comunicación. Bajo esta configuración, la única fuente de aprendizaje para el Estado consiste en el bloque físico-institucional $\mathcal{L}_{F,t}$, el cual integra las señales de los riesgos competitivos del mecanismo $\lambda_j$ y la detección de colusión $d_t$ condicionales a las acciones ejecutadas tras el filtro MDG. El objetivo es verificar si la creencia posterior $\mu_t$ converge al tipo de captor verdadero $\theta_K^\ast$ basándose únicamente en estos desenlaces, validando que el mecanismo es autosuficiente para la identificación.

Como se muestra en la Figura~\ref{fig:cal-eln-par-mu-novoice}, la creencia posterior del tipo verdadero $\mu_t(\theta_K^\ast)$ se aproxima progresivamente a la certeza en los cuatro escenarios, aunque a un ritmo más lento que en el caso base. Dentro del horizonte de $\tau = 150$ períodos ilustrado en las figuras, la creencia sobre el tipo verdadero en el Escenario I (DC) se consolida en la certeza casi total desde los primeros periodos, mientras que en el Escenario II (PAR) y el Escenario III (ELN) la convergencia es evidente antes de finalizar el periodo ilustrado. Por su parte, en el Escenario IV (FARC) el aprendizaje avanza de manera gradual, lo que indica que la convergencia completa requiere horizontes de simulación más largos, del orden de 400 períodos, los cuales se extienden más allá del límite visual de las figuras. Estos resultados ilustran el comportamiento a horizonte finito en la dirección predicha por el Teorema~\ref{thm:posterior-consistency}, sin constituir una validación asintótica directa, dado que los riesgos competitivos físico-institucionales ($\lambda_j$), que dependen de los coeficientes contingentes al tipo y de los controles del Estado $(\alpha_t, \gamma_t)$, exhiben suficiente variación para separar asintóticamente los tipos. Desactivar el canal de voz no bloquea la identificación, pero ralentiza significativamente la velocidad de convergencia en comparación con la Sección~\ref{sec:calibracion}. En el caso de referencia, las características acústicas permitían que el posterior se concentrara dentro de $20$ a $105$ períodos; sin ellas, el Estado requiere una historia de señales físico-institucionales sustancialmente más larga para lograr una precisión equivalente. Esto destaca el papel de la voz como un acelerador informacional y no como una condición necesaria para la identificación.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{Fig_11a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{Fig_11b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}

\medskip
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{Fig_11c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calbeliefheight}{Fig_11d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Creencias bayesianas de cada tipo $\mu_\tau(\theta)$ en los diferentes escenarios bajo exclusión de comunicación de voz (en color en la versión digital).}
\label{fig:cal-eln-par-mu-novoice}
\figurenote{\textit{Nota:} Los paneles (a)-(d) replican los escenarios de referencia pero con el canal de comunicación de voz desactivado. La banda gris marca el límite contrafactual de parada $\tau^{\mathrm{hyp}}$. \textit{Fuente:} Elaboración propia.}
\end{figure}

Al excluir la retroalimentación de la voz, la trayectoria de la presión operativa óptima $\gamma_t^\ast$ y su comparación con los referentes $\gamma_R^\mu$ y $\gamma_N^\mu$, detallados en la Figura~\ref{fig:cal-eln-par-gamma-novoice}, evidencian el impacto de un aprendizaje más lento sobre la persistencia del instrumento. Debido a que la adquisición de información es menos veloz sin la retroalimentación acústica, el Estado se ve obligado a prolongar niveles elevados de presión operativa durante un mayor número de períodos para inducir separación y forzar la revelación a través del hazard de rescate. Por ejemplo, en el caso DC, la presión promedio se sitúa en $\bar\gamma^\ast \approx 0{,}577$ a lo largo de los 150 períodos simulados, frente a los $\bar\gamma^\ast \approx 0{,}564$ registrados en la Sección~\ref{sec:calibracion}. Al igual que en la especificación de referencia, el control dual hace que la trayectoria de presión fluctúe entre ambos referentes antes de converger hacia el benchmark de la rama ejecutada (Negociación) una vez disipada la incertidumbre, aunque la ausencia de señales de voz prolonga significativamente la fase de aprendizaje activo.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_12a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_12b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_12c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_12d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Presión operativa $\gamma_t^\ast$ y puntos de referencia $\gamma_R^\mu$ y $\gamma_N^\mu$ frente a $\tau$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-gamma-novoice}
\figurenote{\textit{Nota:} Los paneles (a)-(d) replican los escenarios con voz pero bajo la exclusión del canal de comunicación. Las líneas punteadas corresponden a los puntos de referencia de Rescate ($\gamma_R^\mu$) y Negociación ($\gamma_N^\mu$) ponderados por creencias. La banda gris marca el límite contrafactual de parada $\tau^{\mathrm{hyp}}$. \textit{Fuente:} Elaboración propia.}
\end{figure}

En una dirección similar, la trayectoria de la interdicción financiera óptima $\alpha_t^\ast$ en relación con los referentes $\alpha_R^\mu$ y $\alpha_N^\mu$, ilustrados en la Figura~\ref{fig:cal-eln-par-alpha-novoice}, refleja una persistencia equivalente bajo la ausencia de comunicación. Al igual que con la presión operativa, el Estado se ve obligado a sostener la interdicción en niveles elevados durante un número absoluto de períodos mayor en todos los escenarios sin voz, compensando así la falta de señales acústicas y evitando la negociación prematura. Por ejemplo, en el Escenario I de DC esta mayor persistencia absoluta no se traduce en un promedio más alto, ya que el retiro paulatino del instrumento tras la convergencia tardía extiende el horizonte relevante hasta los 150 períodos simulados, sobre los cuales el promedio es $\bar\alpha^\ast \approx 0{,}827$; el caso de referencia con voz, en cambio, converge en solo 35 períodos y registra un promedio mayor, de $0{,}896$, precisamente porque esa ventana más corta no alcanza a incluir la fase de descenso posterior a la convergencia.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_13a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_13b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_13c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_13d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Interdicción financiera $\alpha_t^\ast$ y puntos de referencia $\alpha_R^\mu$ y $\alpha_N^\mu$ frente a $\tau$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-alpha-novoice}
\figurenote{\textit{Nota:} Los paneles (a)-(d) replican los escenarios con voz pero bajo la exclusión del canal de comunicación. Las líneas punteadas corresponden a los puntos de referencia de Rescate ($\alpha_R^\mu$) y Negociación ($\alpha_N^\mu$) ponderados por creencias. La banda gris marca el límite contrafactual de parada $\tau^{\mathrm{hyp}}$. \textit{Fuente:} Elaboración propia.}
\end{figure}

Las trayectorias de precisión posterior y presión operativa en el espacio conjunto $(\iota_t,\gamma_t^\ast)$, que se muestran en la Figura~\ref{fig:cal-eln-par-iota-gamma-novoice}, revelan un patrón distinto al de la Sección~\ref{sec:calibracion}. Mientras que allí la retroalimentación de voz y la perturbación óptima de los instrumentos del Estado generaban saltos informacionales marcados en los primeros períodos, aquí la precisión posterior asciende de manera progresiva en todos los escenarios, impulsada únicamente por la perturbación óptima del mecanismo mediante los instrumentos de presión y control ($\gamma_t^\ast$ y $\alpha_t^\ast$). Esta acumulación dinámica de precisión refleja que el aprendizaje descansa en la separación de las tasas de hazard del MDG bajo la inducción estratégica del Estado, sin la ayuda de shocks acústicos discretos. En consecuencia, la presión operativa se reduce paulatinamente a medida que la precisión converge a su límite superior, ilustrando el acoplamiento estrecho entre la acumulación de información física y el retiro del instrumento.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_14a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_14b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_14c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_14d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Trayectoria de precisión posterior y presión operativa $(\iota_t,\gamma_t^\ast)$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-iota-gamma-novoice}

\figurenote{\textit{Fuente:} Elaboración propia.}
\end{figure}

Las trayectorias conjuntas de precisión posterior e interdicción financiera $(\iota_t,\alpha_t^\ast)$, mostradas en la Figura~\ref{fig:cal-eln-par-iota-alpha-novoice}, muestran un patrón análogo al de la presión operativa, en el cual el retiro de la interdicción financiera hacia la rama de información completa óptima es mucho más lento y suave. La ausencia del subsidio informacional acústico elimina las transiciones rápidas de política, forzando al Estado a sostener la interdicción financiera en niveles altos para mantener la compatibilidad de incentivos, de modo que se evite la capitulación, mientras la creencia se actualiza lentamente período a período a partir de los desenlaces físicos.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_15a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_15b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_15c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_15d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Trayectoria de precisión posterior e interdicción financiera $(\iota_t,\alpha_t^\ast)$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-iota-alpha-novoice}

\figurenote{\textit{Fuente:} Elaboración propia.}
\end{figure}

El diagnóstico de restricciones en la Figura~\ref{fig:cal-eln-par-constraints-novoice} confirma que la viabilidad estratégica del mecanismo se preserva al 100\,\% en todos los ciclos simulados, incluso bajo las condiciones de actualización lenta por exclusión de voz. Las restricciones de racionalidad individual de la familia y el captor, denotadas por $IR^F$ e $IR^K$, así como la restricción de compatibilidad de incentivos del captor, $IC^K$, se satisfacen sin excepción. Esto demuestra que la formulación de control dual del Estado es robusta a cambios en la velocidad del aprendizaje, ajustando las trayectorias de política para asegurar que las restricciones de participación y de incentivos de equilibrio sigan siendo viables en cada período.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_16a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_16b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_16c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_16d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Frecuencia de cumplimiento de $\mathrm{IC}^K$, $\mathrm{IR}^K$ e $\mathrm{IR}^F$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-constraints-novoice}

\figurenote{\textit{Fuente:} Elaboración propia.}
\end{figure}

La ganancia informacional esperada $\Delta H_t$ bajo exclusión de comunicación de voz, ilustrada en la Figura~\ref{fig:cal-eln-par-dh-novoice}, se extiende sobre un horizonte considerablemente más largo que en la Sección~\ref{sec:calibracion}. Sin las señales acústicas que aceleraban la convergencia, el Estado depende exclusivamente de las señales físico-institucionales de riesgos competitivos y detección, y el subsidio informacional permanece positivo durante un mayor número de períodos antes de converger a cero. Este patrón es consistente con la mayor duración observada en la concentración posterior de las creencias bajo exclusión de voz, y confirma que el motivo de exploración activa sigue operando incluso cuando el único canal disponible es el físico-institucional.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_17a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_17b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_17c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_17d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Ganancia informacional esperada $\Delta H_t$ frente a $\tau$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-dh-novoice}

\figurenote{\textit{Fuente:} Elaboración propia.}
\end{figure}

La sensibilidad neta de la duración esperada del cautiverio $\kappa_h(\theta_K^\ast,t)$ respecto a la presión operativa, trazada en la Figura~\ref{fig:cal-eln-par-kh-novoice}, reproduce el mismo patrón heterogéneo entre escenarios identificado en la Sección~\ref{sec:calibracion}. En el Escenario~II (PAR) y el Escenario~IV (FARC), $\kappa_h > 0$ en el $87{,}3\,\%$ y el $91{,}3\,\%$ de los ciclos, respectivamente, lo que implica $\partial\,\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast < 0$. En contraste, en el Escenario~I (DC) la sensibilidad vuelve a ser predominantemente negativa, con $\kappa_h < 0$ en el $74{,}7\,\%$ de los períodos, de modo que una mayor presión operativa tiende a prolongar el cautiverio. En el Escenario~III (ELN), la trayectoria se reparte de manera casi uniforme entre ambos regímenes, con $\kappa_h>0$ en el $50{,}0\,\%$ de los períodos. Este resultado confirma que, incluso ante la ausencia del canal de voz, el trade-off estratégico entre presión operativa y duración esperada del cautiverio identificado en la calibración de referencia se mantiene sin alteraciones cualitativas.

\begin{figure}[!ht]
\centering
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_18a_no_voice.pdf}
  \caption{Escenario I: DC , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_18b_no_voice.pdf}
  \caption{Escenario II: PAR , sin voz}
\end{subfigure}
...
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_18c_no_voice.pdf}
  \caption{Escenario III: ELN , sin voz}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
  \centering
  \panelgraphic{\calseriesheight}{Fig_18d_no_voice.pdf}
  \caption{Escenario IV: FARC , sin voz}
\end{subfigure}
\caption{Sensibilidad neta $\kappa_h(\theta_K^\ast,t)$ de la duración esperada del cautiverio respecto a $\gamma_t^\ast$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-kh-novoice}

\figurenote{\textit{Fuente:} Elaboración propia.}
\end{figure}

Los resultados cuantitativos de estas simulaciones contrafactuales, evaluados en el mismo período límite de parada de referencia $\tau^{\mathrm{hyp}}$, se resumen en la Tabla~\ref{tab:calibracion-resumen-novoice}. Al momento de la parada de referencia, la creencia posterior del tipo verdadero se concentra lentamente en DC, alcanzando apenas $\mu_{35} \approx 0{,}222$; en PAR llega a $\mu_{105} \approx 0{,}718$; y en FARC el aprendizaje más lento resulta en una creencia de tan solo $\mu_{20} \approx 0{,}463$; mientras que en ELN la creencia se eleva a $\mu_{75} \approx 0{,}989$. Esto ilustra el retardo en la velocidad de concentración del posterior inducido por la pérdida de la información acústica, forzando un horizonte de interacción mucho más largo antes de lograr el cierre en la mayoría de los escenarios.

\begin{table}[!ht]
\centering
\caption{Resumen de la calibración dinámica bajo exclusión de comunicación de voz}
\label{tab:calibracion-resumen-novoice}
\small
\begin{tabular}{lrrrrrl}
\toprule
$\theta_K^\ast$ & $\mu_0(\theta^\ast)$ & $\mu_{\tau^{\mathrm{hyp}}}(\theta^\ast)$ & $\bar{\alpha}^\ast$ & $\bar{\gamma}^\ast$ & $\tau^{\mathrm{hyp}}$ & $m_{\tau^{\mathrm{hyp}}}$ \\
\midrule
DC & 0.150 & 0.222 & 0.806 & 0.639 & 35 & Pago \\
PAR & 0.350 & 0.718 & 0.913 & 0.617 & 105 & Rescate \\
ELN & 0.050 & 0.989 & 0.791 & 0.662 & 75 & Liberación \\
FARC & 0.150 & 0.463 & 0.751 & 0.706 & 20 & Rescate \\
\bottomrule
\end{tabular}
\par\smallskip\footnotesize \textit{Fuente:} Elaboración propia.
\end{table}
...
Este artículo desarrolla un mecanismo indirecto dinámico para entornos en los que las acciones de equilibrio se agrupan y los tipos no se revelan mediante reportes libres. Respecto a la reducción clásica vía el Principio de Revelación, la contribución es construir el objeto indirecto operativo, compuesto por instrumentos públicos, ruido de implementación endógeno y aprendizaje bayesiano sobre señales generadas por acciones.

A nivel teórico, el núcleo MDG provee la materialización con soporte completo de las intenciones latentes sin un temblor exógeno, manteniendo la regla de Bayes bien definida fuera de la senda. El Teorema de Identificación de Tipos Racionales muestra que la separación recurrente en variación total de las leyes de observación condicionales al tipo implica la singularidad mutua de las medidas de trayectoria y la concentración posterior casi segura en el tipo verdadero en el evento de interacción prolongada $\mathcal{T}$. El Teorema de Implementabilidad Condicional complementa la identificación al garantizar un EBP que satisface la compatibilidad de incentivos y la racionalidad individual siempre que el conjunto factible permanezca no vacío.

A nivel aplicado, el secuestro extorsivo en Colombia mapea el espacio de tipos abstracto a tecnologías criminales estimadas, donde la evidencia de riesgos competitivos y de elección multinomial disciplina los hazards y los priors. Las simulaciones dinámicas de los escenarios de referencia muestran que la regla calibrada de control dual acelera la concentración posterior en el tipo verdadero, sostiene intensidades de instrumentos elevadas pero disciplinadas y preserva la factibilidad de las restricciones IR e IC, en la dirección predicha por la teoría. Asimismo, el análisis contrafactual bajo exclusión del canal de comunicación de voz valida la robustez del mecanismo, al mostrar que la identificación bayesiana es autosuficiente basándose únicamente en las señales físico-institucionales de riesgos competitivos y detección, aunque la velocidad de convergencia posterior se ralentiza significativamente y demanda una historia de interacción más prolongada para alcanzar un nivel equivalente de precisión.

Una agenda de investigación consiste en extender el Teorema de Identificación de Tipos Racionales a otros entornos económicos donde el agrupamiento observacional impide la discriminación pasiva de agentes heterogéneos. En el mercado laboral, donde trabajadores con distinta productividad latente ($\theta$) eligen el mismo nivel de educación o esfuerzo de equilibrio, el proceso MDG puede formalizarse como un diseño de pruebas o asignación estocástica de tareas con ruido endógeno que perturba las intenciones y fuerza la emisión de señales con soporte completo. En la intermediación financiera, frente a prestatarios con variadas probabilidades de impago que imitan esquemas de bajo riesgo, trabajos futuros pueden adaptar el mecanismo para canalizar solicitudes de crédito mediante auditorías ruidosas e incentivos dinámicos, separando los perfiles de riesgo a través de la convergencia bayesiana del prestamista. De manera análoga, en mercados de bienes con calidad asimétrica, la arquitectura del MDG permite descomponer las estrategias de señalización de las firmas, como precios, garantías o certificaciones, en variaciones de soporte completo. En los tres frentes, evaluar la separación en variación total de las leyes de observación resultantes permitirá verificar cómo la filtración de mercado $\mathcal{F}_t^{\mathrm{pub}}$ refina las creencias del Principal, garantizando la convergencia casi segura hacia la verdadera identidad del agente vía \citet{Kakutani1948}.


\section*{Declaración sobre el uso de Inteligencia Artificial Generativa}

En la preparación de este manuscrito, el autor utilizó herramientas de inteligencia artificial generativa, específicamente \emph{Cursor} y \emph{Antigravity}, para asistir en tareas tales como la revisión de literatura y edición de texto; la programación en Lean 4 para la formalización y demostración interactiva de teoremas y lemas teóricos; la programación de código cuantitativo y empírico en Matlab, Stata, y Python; así como el diseño y despliegue en línea de la interfaz del modelo dinámico calibrado a través de GitHub y Streamlit. En consonancia con las directrices éticas académicas, el autor es el único responsable de la integridad del documento, habiendo verificado y validado exhaustivamente toda la información, demostraciones, estimaciones, y códigos provistos o generados por estas tecnologías.
