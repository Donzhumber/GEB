...
\title{Diseño Dinámico de Mecanismos con Aprendizaje Activo y Ruido Endógeno: Identificación de Tipos Racionales en Entornos Desconocidos\tnoteref{t1}}
...
\begin{abstract}
Este artículo estudia un problema de diseño dinámico de mecanismos en el que un planificador, el principal, debe cribar intertemporalmente a un adversario, el agente, cuyas acciones de equilibrio se agrupan, de modo que la observación pasiva no puede separar los tipos. El artículo introduce el proceso Mano de Dios--Guadalupe o MDG, una ley de implementación de ruido endógeno que preserva el soporte completo fuera de la senda de equilibrio sin una perturbación exógena de mano temblorosa. Combinando esta capa con una estructura de riesgos competitivos y un motivo activo de adquisición de información, el artículo establece el Teorema de Identificación de Tipos, mostrando que la separación uniforme en variación total entre la ley de observación de cada tipo fuerza la singularidad mutua de las leyes de trayectoria mediante el criterio de \citet{Kakutani1948}, de modo que las creencias convergen casi seguramente al tipo verdadero del agente. Este resultado de identificación se complementa con un Teorema de Implementabilidad Condicional del Mecanismo que garantiza un equilibrio bayesiano perfecto que satisface la compatibilidad de incentivos y la racionalidad individual siempre que el conjunto factible de políticas sea no vacío. Ambos resultados están verificados computacionalmente en Lean~4. La viabilidad operativa se ilustra con una calibración al secuestro extorsivo en Colombia, presentada como validación aplicada.
\end{abstract}

\begin{keyword}
diseño de mecanismos \sep cribado dinámico \sep aprendizaje bayesiano \sep aprendizaje activo \sep identificación \\
...
El comportamiento criminal observado a menudo se agrupa exactamente en el punto en el que la política pública necesita discriminar. Una solicitud de pago, un lapso de silencio, una prueba de vida parcial o una escalada violenta pueden ser compatibles con varias tecnologías organizacionales. Cuando la misma señal pública puede ser generada por organizaciones insurgentes disciplinadas, bandas criminales de corto horizonte o estructuras posconflicto fragmentadas, un planificador pasivo enfrenta un problema de bloqueo informacional en el que la política que minimiza las pérdidas actuales preserva precisamente el agrupamiento que impide la identificación del tipo el día de mañana. El diseño de mecanismos, concebido axiomáticamente como la arquitectura analítica para implementar asignaciones sociales o privadas bajo las restricciones de participación voluntaria y racionalidad secuencial de los agentes, provee el marco estructural para formular reglas, instituciones y contratos capaces de alinear los incentivos privados con los objetivos normativos de un planificador. Este artículo introduce un mecanismo indirecto dinámico para la disuasión del secuestro extorsivo en tiempo discretizado bajo información incompleta, modelando la interacción estratégica e intertemporal entre el Estado ($S$), el captor ($K$) y la familia de la víctima ($F$).

La pregunta central es cómo un planificador puede diseñar un mecanismo de cribado activo cuando las acciones observables caen en regiones de agrupamiento, una dificultad análoga a los problemas de negociación bajo información privada de \citet{Fearon1995Rationalist}. El planificador es el Estado. Sus instrumentos son continuos: interdicción financiera $\alpha_t\in[0,1]$ y presión operativa $\gamma_t\in[0,1]$. El adversario tiene un tipo privado $\theta_K\in\ThetaK$. El problema de política no consiste solamente en elegir entre rescate y negociación, sino en calibrar $(\alpha_t,\gamma_t)$ de modo que la ley inducida de las señales diarias se vuelva informativa sobre la tecnología criminal latente. Colombia provee la base empírica de este análisis, no como narrativa histórica. Sus episodios de secuestro ofrecen un entorno excepcional donde la dinámica macro de largo plazo, la duración micro y la heterogeneidad de los perpetradores revelan conjuntamente los límites informacionales de la disuasión.

\subsection{Marco Teórico e Innovaciones}\label{subsec:framing}

Este artículo se sitúa en la frontera abierta del diseño dinámico de mecanismos. \citet{Gibbard1973} y \citet{Myerson1979IC,Myerson1981} demostraron que todo mecanismo indirecto puede representarse como un mecanismo directo y veraz, resultado conocido como el Principio de Revelación, que reduce el diseño a restricciones de Compatibilidad de Incentivos ($IC$) y Racionalidad Individual ($IR$). \citet{MyersonSatterthwaite1983}, \citet{Rochet1987} y \citet{RochetChone1998} delimitaron las fronteras alcanzables en el caso estático; \citet{KadanRenySwinkels2017} estableció las condiciones generales de existencia para mecanismos óptimos en entornos principal-agente, proveyendo el fundamento topológico para las hipótesis de factibilidad que siguen. Estos resultados motivan el giro hacia los mecanismos dinámicos de \citet{CourtyLi2000}, \citet{EsoSzentes2007} y \citet{PavanSegalToikka2014}. Esta literatura requiere dos extensiones estructurales para su aplicación en entornos institucionales adversariales: la garantía de robustez fuera de la senda de equilibrio, que preserva una regla de Bayes bien definida ante desviaciones inesperadas de equilibrio, y la incorporación de la exploración informacional activa (\textit{active learning}), en línea con la adquisición dinámica óptima de información de \citet{Zhong2022}, que impide que una política miope congele los incentivos y bloquee la identificación del tipo del adversario. La economía del crimen de \citet{Becker1968} y \citet{Ehrlich1973}, extendida al secuestro por \citet{Selten1977Kidnapping}, \citet{Iqbal2017Kidnapping} y \citet{Mejia2000Secuestro}, y prolongada por trabajos empíricos recientes como \citet{Detotto2014Ransom} sobre la duración del cautiverio mediante análisis de supervivencia, \citet{Forest2012Kidnapping} sobre la heterogeneidad ideológica y organizacional de los perpetradores, y \citet{Shortland2019Kidnap} sobre la gobernanza privada del mercado de rescates, documenta la dinámica operativa del crimen con creciente detalle pero no tiende el puente hacia la arquitectura bayesiana de identificación de tipos requerida por un planificador activo. El presente artículo ocupa exactamente ese vacío y realiza tres contribuciones interconectadas. La primera es la implementación estocástica a través del proceso Mano de Dios-Guadalupe (MDG). A diferencia de los mecanismos convencionales que asumen realizaciones determinísticas, el modelo adopta un enfoque indirecto que modela los instrumentos del Estado tal como se despliegan en la realidad institucional. En la especificación base con un piso residual positivo, el filtro MDG, justificado axiomáticamente como la ley única compatible con soporte completo, dominancia y máxima entropía bajo el teorema de caracterización de \citet{Jaynes1957Information}, introduce un ruido logit con una temperatura híbrida entre la intención estratégica latente y la acción materializada en el terreno. Esta capa estocástica comparte el propósito analítico de los refinamientos clásicos de \citet{Selten1975Perfectness}, \citet{KrepsWilson1982} y \citet{FudenbergTirole1991} al garantizar soporte completo sobre las ramas del juego y mantener el operador bayesiano bien definido fuera de la senda de equilibrio (\textit{off-path}). Sin embargo, en comparación con esos enfoques, el proceso MDG constituye una fricción intrínsecamente endógena y una condición necesaria para la materialización de la rama operativa.

La segunda contribución es la fusión de riesgos competitivos con aprendizaje y control dual. El artículo incorpora la ley empírica de movimiento en una estructura de riesgos competitivos proporcionales en tiempo discreto inspirada en \citet{Cox1972}, separando la duración y el modo de terminación como objetos distintos pero conjuntamente informativos. El operador $\xi_j(t)$ actúa como la cuota de mercado de cada riesgo y ortogonaliza la gobernanza de \emph{cuándo} de la gobernanza de \emph{cómo}, de modo que las verosimilitudes bayesianas de alta frecuencia se actualizan sin distorsionar la persistencia subyacente del cautiverio. Para evitar el estancamiento informacional, el artículo adapta la intuición de control dual de \citet{Feldbaum1965} e incorpora un motivo de exploración activa a través de la entropía de Shannon \citet{Shannon1948} y la adquisición dinámica óptima de información de \citet{Zhong2022}. El Estado internaliza que sus acciones presentes alteran la información futura y acepta una penalización de pérdida en el corto plazo para aplicar un subsidio informacional que obliga al captor a revelar su huella estructural.

La tercera contribución es el Teorema de Identificación de Tipos Racionales en Entornos Desconocidos. El resultado establece que si la huella estructural $\beta_{K,j}(\theta_K)$ de cada tipo induce separación uniforme en variación total entre las leyes de observación diarias, entonces el criterio de \citet{Kakutani1948} para productos de medidas de probabilidad implica singularidad mutua de las leyes de trayectoria y, en el evento de cautiverio prolongado $\mathcal{T}=\{\tau=\infty\}$, el vector de creencias bayesianas $\mu_t$ converge casi seguramente a la medida de Dirac sobre el tipo verdadero, $\Dirac_{\theta_K^\ast}$, de acuerdo con los axiomas de \citet{BlackwellDubins1962}. La demostración parte de la estabilidad asintótica de una martingala acotada en el sentido de \citet{Doob1953Stochastic} y apela al criterio de \citet{Kakutani1948} para cerrar la identificación. El teorema convierte una condición local de separación diaria en condiciones globales de identificación posterior condicionadas a $\mathcal{T}$. El Teorema de Implementabilidad Condicional del Mecanismo en EBP complementa este resultado: bajo la hipótesis de que el conjunto factible $\Gamma_t(\mu_t)$ permanece no vacío, existe un EBP en el que la regla óptima del Estado satisface simultáneamente $IC^K$, $IR^K$ e $IR^F$.

El resto del artículo procede de la siguiente manera. La Sección~\ref{sec:institutional} traduce la evidencia colombiana en restricciones sobre tipos y costos; la Sección~\ref{sec:pertinencia} muestra cómo los modelos empíricos anclan el prior del mecanismo; la Sección~\ref{sec:model} desarrolla el modelo dinámico, el programa del Estado y el teorema de identificación; y la Sección~\ref{sec:calibracion} presenta la calibración y las simulaciones ELN--PAR. Los Apéndices Suplementarios contienen los resultados macroeconométricos detallados, la microestructura y los riesgos competitivos, y las demostraciones formales con verificación en Lean~4.


\section{Anclaje Empírico del Espacio de Parámetros}\label{sec:institutional}

Entre 1964 y 2025, Colombia registró 61.341 casos de secuestro \citep{cnmh_datos}, concentrados en municipios específicos y estrechamente ligados a la evolución del conflicto armado interno; la creación en 1996 de los Grupos de Acción Unificada por la Libertad Personal (GAULA, \citealt{ley282_1996}) marcó la respuesta institucional del Estado ante esta dinámica, y la variación resultante en capacidad coercitiva, reforma legal y estructura organizacional entre los grupos armados es precisamente la fuente de la heterogeneidad de tipos que el mecanismo está diseñado para cribar. Esta motivación aplicada se desarrolla extensamente, junto con el análisis macroeconométrico subyacente (un modelo de Vectores de Corrección de Errores y un modelo de Tendencia Lineal Local de la tendencia estocástica del secuestro, formalmente reconciliados por el corolario de equivalencia de tendencias), en el Apéndice A; la estimación microeconométrica completa que sustenta la tabla siguiente se presenta en el Apéndice B.

Los parámetros que alimentan el espacio de estados y las tecnologías del mecanismo se obtienen de un modelo Logit Multinomial (MNL) de los desenlaces de los casos y de un modelo de riesgos proporcionales específicos por causa de \citet{Cox1972}, estimados sobre microdatos a nivel individual del Centro Nacional de Memoria Histórica \citep{cnmh_datos} ($n=1{,}125$ muestra analítica; especificación completa, pruebas de robustez y diagnósticos de selección muestral en el Apéndice B). La Tabla~\ref{tab:hazard-prior-summary} consolida la huella tecnológica resultante por tipo para los cuatro tipos de captor $\ThetaK=\{\text{DC},\text{PAR},\text{ELN},\text{FARC}\}$.

\begin{table}[H]
\centering
\caption{Huella consolidada de hazard y desenlace por tipo (DC, PAR, ELN, FARC)}
\label{tab:hazard-prior-summary}
\begin{tabular}{lccc}
\toprule
Tipo $\theta_K$ & Margen de mortalidad (MNL) & Razón de hazard de pago (Cox, ref.\ FARC) & Rescate acum.\ @ día 100 (Cox) \\
\midrule
FARC & 0.002 & 1.00 (referencia) & 0.27 \\
ELN  & 0.007 & $\approx 1.00$ (n.s., $p=0.317$) & 0.28 \\
PAR  & 0.026 & $4.28$ ($p<0.001$) & 0.33 \\
DC   & 0.071 & $1.71$ ($p=0.021$) & n.r.\ (véase Apéndice~B) \\
\bottomrule
\end{tabular}
\end{table}

La correspondencia entre estas estimaciones y el modelo teórico de la Sección~\ref{sec:model} es directa. Los márgenes de mortalidad ordenan la utilidad de reserva de la liberación $U^K_{\mathrm{rel}}(\theta)=-\kappa_{\mathrm{rel}}(\theta)$ entre tipos, dado que las organizaciones con menor capacidad logística de retención (mayor mortalidad) enfrentan un cálculo correspondientemente distinto en la restricción de participación $IR^K$. Las razones de hazard de Cox se trasladan al mecanismo como las intensidades efectivas específicas por tipo $\tilde\lambda_j(t\mid\theta_K)$ que gobiernan la señal diaria de riesgos competitivos $\zeta_t$ y, a través de ella, la actualización bayesiana de $\mu_t$; su separación empírica entre tipos es la contraparte observacional de la hipótesis de separación uniforme en variación total del Lema~\ref{lem:hazard-separation}, sobre la cual descansa el Teorema~\ref{thm:posterior-consistency}. Las creencias iniciales específicas por escenario $\mu_0(\theta\mid z,\theta_V)$ utilizadas en la calibración de la Sección~\ref{sec:calibracion} se reportan allí junto con cada escenario simulado.

\section{Pertinencia del Marco Empírico para el Modelo del Mecanismo}\label{sec:pertinencia}

El modelo teórico de la Sección~\ref{sec:model} requiere dos insumos empíricos para no depender de supuestos ad hoc: un entorno macroeconómico persistente que discipline los costos de retención y una estructura de tipos con priors, tecnologías y hazards diferenciados. El sistema VEC, la descomposición de Gonzalo--Granger y el corolario de equivalencia de tendencias (Apéndice A) proveen el primer insumo. La cointegración entre $K^E$, $K^{NE}$ y $r$ establece que el secuestro extorsivo no evoluciona como una serie aislada, sino como parte de un sistema de largo plazo en el que la capacidad coercitiva y las condiciones macrofinancieras actúan como fuerzas permanentes. La equivalencia entre la tendencia VEC--GG y la tendencia LLT evita que la calibración dependa de una única especificación econométrica, mientras que la estructura de ajuste del VEC muestra que $K^E$ es el margen que absorbe los desequilibrios. Por lo tanto, el vector de estado del mecanismo, el conjunto factible $\Gamma_t(\mu_t)$ y las restricciones $IC^K$ e $IR^{K,F}$ están anclados en regularidades persistentes, no en una normalización puramente teórica.

Sobre esta base, el MNL y el modelo de Cox conectan la evidencia microeconómica con la distribución inicial $\mu_0\in\Delta(\ThetaK)$ y los vectores de tecnología $\beta_{K,j}(\theta_K)$. El MNL provee la huella estática de cada tipo al relacionar perpetrador, víctima, contexto y tácticas con la distribución condicional de desenlaces; así, la creencia inicial y los pagos terminales se fijan con pesos observados. El modelo de Cox provee la dimensión dinámica: sus hazards específicos por causa se trasladan al mecanismo como tasas $\lambda_j(\theta_K,t)$ que gobiernan la señal diaria $\zeta_t$, actualizan el posterior $\mu_t$ y determinan la velocidad del aprendizaje. La separación empírica de esas tasas entre organizaciones es la base observacional del Lema~\ref{lem:hazard-separation} y del criterio de \citet{Kakutani1948}: si las trayectorias inducidas por los tipos generan leyes suficientemente distintas, el posterior puede concentrarse en el tipo verdadero. En conjunto, el bloque empírico convierte los datos macro y micro en priors, costos, hazards y restricciones directamente utilizables por el mecanismo dinámico. El protocolo explícito de traducción entre estimadores y parámetros estructurales se detalla al comienzo de la Sección~\ref{sec:calibracion}.

\section{El Modelo Teórico}\label{sec:model}

Esta sección formaliza el mecanismo para la disuasión y el desmantelamiento del secuestro extorsivo mediante un enfoque indirecto. Si bien el Principio de Revelación de \citet{Gibbard1973} y \citet{Myerson1979IC,Myerson1981} garantiza la equivalencia con un mecanismo directo, ruta empleada por \citet{MyersonSatterthwaite1983} para delimitar la frontera de eficiencia del intercambio bilateral, por \citet{Rochet1987} para caracterizar las condiciones de implementabilidad en entornos cuasilineales con tipos multidimensionales, y por \citet{RochetChone1998} para resolver el problema de cribado no lineal multidimensional, esta ruta no es viable aquí porque el captor nunca revela voluntariamente su tipo, en línea con los incentivos a falsear información privada de \citet{Fearon1995Rationalist}, y los instrumentos observables, interdicción financiera $\alpha_t\in[0,1]$ y presión operativa $\gamma_t\in[0,1]$, perderían su anclaje empírico al colapsar en mensajes abstractos de revelación. El proceso MDG operacionaliza el enfoque indirecto mediante un ruido logit con una temperatura híbrida entre la intención estratégica latente y la acción materializada; en la especificación base con un piso residual positivo, garantiza soporte completo en todas las ramas del juego de forma endógena a la dinámica operativa, a diferencia de \citet{Selten1975Perfectness}, \citet{KrepsWilson1982} y \citet{FudenbergTirole1991}, quienes logran el mismo propósito mediante perturbaciones axiomáticas externas sobre las estrategias, sistemas de creencias consistentes con la racionalidad secuencial y reglas de actualización bayesiana en EBP. Esta garantía de soporte completo es la condición que permite que el EBP discipline las creencias sobre $\theta_K$ mediante la actualización de Bayes y la consistencia secuencial, de modo que la acumulación de señales públicas generadas por riesgos competitivos asintóticamente separables fuerza la convergencia de la distribución posterior hacia la identidad estructural del tipo verdadero, resultado formalizado en el Teorema~\ref{thm:posterior-consistency}.

\subsection{Definición del mecanismo}\label{subsec:def-mecanismo}

Sea $\mathcal{J}=\{S,K,F\}$ el conjunto de jugadores, donde $S$ representa al Estado, $K$ al captor y $F$ a la familia. Cada episodio se caracteriza por el vector de tipos $\theta=(\theta_K,\theta_F,\theta_V,\theta_S)\in\Theta$, donde $\ThetaK=\{\text{DC},\text{PAR},\text{ELN},\text{FARC}\}$ corresponde al tipo del captor, a saber, delincuencia común, grupos paramilitares, Ejército de Liberación Nacional y Fuerzas Armadas Revolucionarias de Colombia; $\Theta_F=\{\text{Alta},\text{Baja}\}$ representa la capacidad de pago de la familia; $\Theta_V=\{\text{Público},\text{Privado}\}$ representa el perfil de la víctima; y $\Theta_S=\{\text{Duro},\text{Laxo}\}$ representa el entorno institucional. Cada episodio también se localiza en una zona geográfica observable $z\in\mathcal{R}$, donde $\mathcal{R}$ agrupa cinco regiones (Metropolitana, Andina, Caribe, Pacífica y Oriente-Selva); $z$ no es un tipo estratégico privado sino una condición inicial pública que disciplina el prior sobre el tipo del captor. Solo $\theta_K$ es información privada de $K$; los demás componentes y $z$ son observables al abrirse el episodio, con una creencia inicial $\mu_0(\theta\mid z,\theta_V)=\Prob(\theta_K=\theta\mid z,\theta_V)\in\Delta(\ThetaK)$.
...
\]
donde $\mathcal{F}_t^{\mathrm{pub}}$ es la filtración pública al inicio de $t$, generada por la $\sigma$-álgebra de la localización geográfica inicial $z$ y las señales públicas $\zeta_0,\ldots,\zeta_{t-1}$; cada señal $\zeta_s=(\alpha_s^\ast,\gamma_s^\ast,\tilde{a}_s^F,\tilde{a}_s^K,\tilde{a}_s^S,m_s,d_s)$ agrega todos los observables del período $s$ y constituye el soporte informacional sobre el cual el Estado actualiza sus creencias sobre $\theta_K$; de modo que los instrumentos $(\alpha_t^\ast,\gamma_t^\ast)$ se calibran sobre inferencias correctas acerca del tipo del captor.
...
\noindent En cada período $t$, la familia elige $a_t^F\in\mathcal{A}^F=\{a_{\mathrm{coop}},a_{\mathrm{col}}\}$ maximizando su utilidad esperada sobre la incertidumbre respecto al tipo del captor, ponderada por su conjunto de información privada $\mathcal{I}_t^F$. En la rama de cooperación, la familia sigue la vía institucional, pagando el costo $e_t(\gamma_t,\theta_F)$, que es convexo y creciente en $\gamma_t$ con variación asimétrica entre tipos de familia que genera una propiedad de cruce único análoga a la del captor; a cambio, obtiene el valor esperado de la supervivencia de la víctima ponderado por $V_L>0$:
\begin{equation}
...
\subsubsection*{Esbozo de la Demostración e Intuición Matemática}

La demostración, presentada en su totalidad en el Apéndice~3, ensambla los tres ingredientes ya introducidos anteriormente; aquí se esboza la lógica que los conecta para que el resultado sea legible sin salir del texto principal.

\emph{(1) Las huellas tecnológicas persistentes separan las leyes de señal diarias.} Los hazards específicos por causa $\tilde\lambda_j(t\mid\theta_K)$ que gobiernan la señal diaria de riesgos competitivos $\zeta_t$ dependen de $\theta_K$ solo a través del vector de coeficientes $\beta_{K,j}(\theta_K)$ (Sección~\ref{sec:model}); siempre que dos tipos $\theta,\theta'$ tengan $\beta_{K,j}(\theta)\neq\beta_{K,j}(\theta')$ para alguna causa $j$, las distribuciones de desenlace de un período inducidas difieren puntualmente, lo que por el Lema~\ref{lem:hazard-separation} parte~\textup{(i)} produce una brecha de variación total estrictamente positiva $\mathrm{TV}(p_\theta^{(t)},p_{\theta'}^{(t)})\ge\varepsilon>0$ en todo período en el que la asimetría tecnológica está activa. Dado que $\beta_{K,j}(\theta_K)$ es un parámetro estructural e invariante en el tiempo del tipo, y no un artefacto de los instrumentos de un período particular, esta separación local recurre a lo largo de un subconjunto infinito de períodos siempre que el episodio permanezca abierto (parte~\textup{(ii)}), en lugar de desvanecerse tras un número finito de observaciones.

\emph{(2) La temperatura del MDG gobernada por entropía no borra la señal.} La acción ejecutada $\tilde a_t^i$ es una implementación ruidosa de la intención latente $a_t^{i\ast}$ a través del logit de temperatura híbrida del proceso MDG, con temperatura $T_t=T_0\max\{H(\mu_t)/H(\mu_0)\,e^{-\eta_{\mathrm{cal}}t},\underline{c}\}$ acotada por debajo por el piso $\underline c>0$. Dado que $T_t$ está acotada lejos de cero y de infinito para todo $t$ finito, el ruido del MDG garantiza soporte completo (Axioma A1) sin asignar nunca a la señal de riesgos competitivos una ley degenerada y plenamente determinista; el ruido diluye pero no destruye la brecha de variación total establecida en el paso~(1), de modo que el margen de separación $\varepsilon_\theta$ del Lema~\ref{lem:hazard-separation} permanece estrictamente positivo incluso tras pasar por la capa de implementación.

\emph{(3) La convergencia de martingala y el criterio de Kakutani cierran la identificación.} Por la Definición~\ref{def:beliefs-martingale}, $\mu_t(\theta)$ es, para cada $\theta$ fijo, una martingala acotada respecto a la filtración pública $(\mathcal F_t^{\mathrm{pub}})_{t\ge0}$; el teorema de convergencia de martingalas de Doob (\citealt{Doob1953Stochastic}) garantiza entonces un límite casi seguro $\mu_\infty(\theta)\in[0,1]$ (Lema~\ref{lem:asymptotic-beliefs}). Este argumento de límite por sí solo no determina \emph{dónde} se concentra la masa. Eso lo provee la separación recurrente en variación total de los pasos~(1)--(2): bajo la condición~(C3), infinitos períodos con $\mathrm{TV}\ge\underline\varepsilon_\theta>0$ fuerzan que la afinidad de Hellinger entre las leyes de trayectoria de $\theta$ y $\theta^\ast$, $\rho_t=\sum_y\sqrt{p_\theta^{(t)}(y)\,p_{\theta^\ast}^{(t)}(y)}\le 1-\underline\varepsilon_\theta^2/2<1$ en cada uno de esos períodos, se desvanezca en el producto infinito $\prod_t\rho_t\to0$; el criterio de \citet{Kakutani1948} para productos infinitos de medidas de probabilidad convierte entonces este producto de afinidades que se desvanece en singularidad mutua de las leyes de trayectoria $\ProbC\theta(\cdot\mid\mathcal T)$ y $\ProbC{\theta^\ast}(\cdot\mid\mathcal T)$. La singularidad mutua, junto con el límite de martingala del paso~(3), fuerza a que la masa posterior del tipo falso colapse a cero ($\mu_t(\theta)\to0$), y la normalización del símplex sobre el espacio de tipos finito $\Delta(\ThetaK)$ produce entonces $\mu_t(\theta^\ast)\to1$.

\paragraph{Formalización en Lean~4.} El núcleo de teoría de la medida de este argumento está verificado computacionalmente en un archivo complementario independiente de Lean~4 (\texttt{Appendix3Proofs.lean}, distribuido con el suplemento), construido sobre las bibliotecas de probabilidad y martingalas de \texttt{Mathlib} (\texttt{Mathlib.Probability.Martingale.Basic}, \texttt{Mathlib.Probability.Martingale.Convergence}, \texttt{Mathlib.Probability.Process.Filtration}, y los módulos de esperanza condicional y seminorma $L^p$ de \texttt{Mathlib.MeasureTheory}). El archivo refleja las ocho secciones del Apéndice~3 y marca cada resultado como \texttt{[Proved]} (verificado computacionalmente) o \texttt{[Hypothesis]} (un supuesto estructural o clásico enunciado como interfaz, no demostrado en Lean). El paso de martingala está completamente verificado computacionalmente: la acotación, la propiedad de torre, y la convergencia casi segura a un límite con valores en $[0,1]$ (Lema~\ref{lem:asymptotic-beliefs}) se demuestran a partir del teorema de convergencia de submartingalas de Mathlib sin hipótesis adicionales. El álgebra de riesgos competitivos del paso~(1) está igualmente demostrada por completo: que $\mathrm{predictiveMass}$ define una masa de probabilidad genuina, y que coeficientes de Cox distintos $\beta_{K,j}(\theta)\neq\beta_{K,j}(\theta')$ fuerzan un margen de variación total estrictamente positivo, son teoremas verificados computacionalmente, no supuestos. Dos componentes del argumento se registran como interfaces \texttt{[Hypothesis]} explícitas en lugar de formalizarse por completo: la caracterización de máxima entropía del logit del MDG (Axioma~A3, que requiere el cálculo variacional de \citealt{Jaynes1957Information} que no forma parte de Mathlib), y el paso clásico de teoría de la medida del propio criterio de Kakutani (\texttt{kakutaniMutualSingularity}), que se enuncia como hipótesis en lugar de redemostrarse desde primeros principios dentro de Lean. La formalización certifica, por lo tanto, el puente probabilístico novedoso específico de este mecanismo, la separación recurrente de Cox que implica identificación asintótica, mientras trata apropiadamente dos resultados clásicos de la literatura matemática como hechos importados en lugar de re-derivarlos.

El Lema~\ref{lem:hazard-separation} y el Teorema~\ref{thm:posterior-consistency} formulan la identificación en el lenguaje del mecanismo de riesgos competitivos. El Apéndice~3 presenta versiones generalizadas que aíslan el núcleo probabilístico del argumento: un lema abstracto sobre productos de experimentos y singularidad mutua bajo separación recurrente en variación total, siguiendo a \citet{Kakutani1948}, y un teorema de consistencia posterior que reformula la misma conclusión bayesiana sustituyendo la hipótesis sobre $\beta_{K,j}(\theta_K)$ por esas condiciones de teoría de la medida. La parte~\textup{(ii)} del lema operativo actúa como un puente verificable entre ambos niveles; las demostraciones completas y la verificación algebraica parcial en Lean~4 se reportan en el suplemento.
...
\smallskip
\noindent EBP. El par $(s,\mu)$ es un EBP si se cumplen las condiciones \textup{(a)} y \textup{(b)}:
\begin{enumerate}
  ...
  \]
  donde $L_t$ es la pérdida social condicional al tipo, $\Pi_{t,a^S}^S$ es la penalización por desviación del punto de referencia bayesiano de la rama elegida, y $\mathcal{U}^K_t$, $\mathcal{U}^F_t$ son las utilidades esperadas del captor y de la familia en $t$.
...
\end{equation}
Apoyándose en el marco de existencia de \citet{KadanRenySwinkels2017}, el Teorema~\ref{thm:ebp-implementabilidad} trata la no vacuidad de $\Gamma_t(\mu_t)$ como la condición primitiva de factibilidad robusta que garantiza la existencia de un EBP implementable. La demostración se presenta en el Apéndice~3.
\end{definition}
...
\begin{list}{}{\setlength{\labelwidth}{2.6em}\setlength{\labelsep}{0.7em}\setlength{\leftmargin}{3.3em}\setlength{\itemindent}{0pt}\setlength{\listparindent}{0pt}\setlength{\parsep}{0pt}\setlength{\itemsep}{0.35\baselineskip}}
  ...
  \]
  es convexa, y la correspondencia $\mu_t\mapsto\Gamma_t(\mu_t)$ es continua \textup{(}hemicontinua superior e inferiormente\textup{)} y de valor compacto en $\Delta(\ThetaK)$. La no vacuidad es una condición primitiva de compatibilidad simultánea entre $IC^K$, $IR^K$ e $IR^F$ \citep{KadanRenySwinkels2017}.
...
\noindent\textit{Supuestos de trabajo.} Se imponen dos supuestos de trabajo. \textup{(S1)} La derivada $\partial\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast$ es una derivada parcial: $\gamma_t^\ast$ se perturba en un único período $t$ mientras la secuencia de instrumentos $\{\gamma_s^\ast\}_{s\neq t}$ se mantiene fija; la duración esperada se expresa como $\mathbb{E}[\tau\mid\theta_K]=\sum_{s=0}^{\infty}S(s\mid\theta_K)$ con $S(s\mid\theta_K)=\prod_{r=1}^{s}p_{\mathrm{Cont},r\mid\theta_K}$, de modo que solo los factores $S(s)$ con $s\geq t$ dependen de $\gamma_t^\ast$. \textup{(S2)} La presión operativa desplaza cada intensidad proporcionalmente a su nivel: $\partial\tilde{\lambda}_j(t\mid\theta_K)/\partial\gamma_t^\ast=\zeta_{\gamma,j}\,\tilde{\lambda}_j(t\mid\theta_K)$, donde $\zeta_{\gamma,j}:=\partial\ln\tilde{\lambda}_j(t\mid\theta_K)/\partial\gamma_t^\ast$ es la semielasticidad de la intensidad $j$ respecto a $\gamma_t^\ast$; $\lambda_4$ es invariante a $\gamma_t^\ast$ porque es exógena.
...
\normalfont\normalsize
Esta sección calibra el mecanismo dinámico de la Sección~\ref{sec:model} en dos episodios simulados de horizonte finito. El ejercicio combina priors y diagnósticos macro y microeconométricos para estudiar cuatro objetos: la evolución de las creencias $\mu_\tau(\theta)$, las señales observadas por el Estado, la respuesta de los instrumentos $(\alpha_t,\gamma_t)$, y el cumplimiento de las restricciones IR e IC. El \emph{Escenario~I} corresponde al ELN, con $\theta_K^\ast=\mathrm{ELN}$, Estado laxo, familia de baja riqueza, víctima pública, y localización Caribe; el \emph{Escenario~II} corresponde al PAR, con $\theta_K^\ast=\mathrm{PAR}$, Estado laxo, familia de alta riqueza, víctima privada, y localización Oriente-Selva. Ambos se simulan sobre un horizonte fijo $\bar{T}=150$ períodos (un parámetro de calibración distinto del tiempo de parada endógeno $\tau$ definido en~\eqref{eq:stopping-time}), y la política óptima se calcula a partir de la minimización del costo social del Estado. En el orden $(\mathrm{DC},\mathrm{PAR},\mathrm{ELN},\mathrm{FARC})$, los priors iniciales son $\mu_0^{\mathrm{ELN}}=(0.25,0.35,0.05,0.35)$ y $\mu_0^{\mathrm{PAR}}=(0.55,0.05,0.35,0.05)$. En las figuras, la barra gris marca el tiempo de parada realizado $\tau$ (el primer período con $m_\tau\in\mathcal{M}_{\mathrm{term}}$); la trayectoria posterior a esa barra es una extensión contrafactual bajo un estado absorbente (la ley del mecanismo condicionada a la no absorción) que ilustra la dinámica de aprendizaje e identificación independientemente del momento de cierre, sin constituir una trayectoria real de episodio.
...
\end{subfigure}
\caption{Creencias bayesianas de cada tipo $\mu_\tau(\theta)$ (ELN: $\tau\leq 150$; PAR: $\tau\leq 150$).}
\label{fig:cal-eln-par-mu}
\figurenote{\textit{Nota:} El panel (a) corresponde al Escenario~I: $\theta_K^\ast=\mathrm{ELN}$, $\theta_S$ laxo, $\theta_F$ de baja riqueza, $\theta_V$ público, y $z$ Caribe. El panel (b) corresponde al Escenario~II: $\theta_K^\ast=\mathrm{PAR}$, $\theta_S$ laxo, $\theta_F$ de alta riqueza, $\theta_V$ privado, y $z$ Oriente-Selva. La banda gris marca $\tau^{\mathrm{hyp}}$, el primer período en el que el resultado del mecanismo difiere de la continuación.}
\end{figure}
\vspace{-8pt}
El posterior del tipo verdadero permite evaluar la capacidad del mecanismo para separar tipos a partir de la secuencia de señales. En el escenario ELN, reportado en la Figura~\ref{fig:cal-eln-par-mu}, la masa posterior se desplaza gradualmente hacia ELN hasta alcanzar $\mu_{150}(\mathrm{ELN})\approx 1{,}000$; esto implica que el mecanismo acumula identificación incluso en un entorno con menor riqueza familiar y una víctima pública. En PAR, el posterior del tipo verdadero parte de $\mu_0(\mathrm{PAR})=0{,}050$ y alcanza $\mu_{150}(\mathrm{PAR})\approx 1{,}000$; la trayectoria no es monótona en la fase inicial, porque el evento combina señales tempranas compatibles con otros tipos y luego desplaza la masa hacia PAR. Las barras grises marcan el tiempo de parada realizado ($\tau=16$ en ELN, $\tau=5$ en PAR); la extensión contrafactual subsiguiente muestra que la velocidad de aprendizaje y el momento de absorción son objetos distintos del modelo. En conjunto, la figura ilustra el comportamiento a horizonte finito: la masa posterior se desplaza hacia el tipo verdadero a medida que las señales acumulan separación, en la dirección predicha por el Teorema~\ref{thm:posterior-consistency} bajo $\mathcal{T}$, sin constituir evidencia directa del resultado asintótico; la velocidad de convergencia depende del entorno institucional, territorial, y de visibilidad del caso.
...
\figurenote{\textit{Nota:} El panel (a) muestra la trayectoria de presión operativa en el Escenario~I (ELN). El panel (b) muestra la trayectoria equivalente en el Escenario~II (PAR). La línea punteada corresponde al punto de referencia de Rescate $\gamma^R$. La banda gris marca $\tau^{\mathrm{hyp}}$.}
\end{figure}
La presión operativa óptima permite evaluar cómo ajusta el Estado la intensidad de la intervención a medida que aprende sobre el tipo del captor. La Figura~\ref{fig:cal-eln-par-gamma} compara la trayectoria de $\gamma_t^\ast$ con el punto de referencia de Rescate $\gamma^R$, que toma el valor $\gamma^R=0{,}92$ en ELN y $\gamma^R=0{,}86$ en PAR. En el escenario ELN, $\gamma_t^\ast$ se ubica en promedio alrededor de $\bar\gamma^\ast\approx 0{,}908$ y permanece cercano, aunque por debajo, del punto de referencia de Rescate, consistente con un entorno Caribe y Estado laxo. En PAR, la presión permanece cerca del punto de referencia de Rescate y alcanza $\bar\gamma^\ast\approx 0{,}868$, consistente con un entorno Oriente-Selva y Estado laxo donde la intervención conserva valor incluso cuando el desenlace focal del evento es el pago. En ambos casos, $\gamma_t^\ast$ no replica mecánicamente ni la rama de Rescate ni la de Negociación: el control dual combina la explotación del instrumento y la información esperada, de modo que la serie fluctúa entre puntos de referencia según el riesgo material del episodio y el valor de separar tipos.

La interdicción financiera óptima resume la intensidad con la que el Estado restringe la capacidad de pago y la coordinación económica durante el episodio. La Figura~\ref{fig:cal-eln-par-alpha} compara la trayectoria de $\alpha_t^\ast$ con el punto de referencia de Rescate $\alpha^R$, que toma el valor $\alpha^R=0{,}90$ en ELN y $\alpha^R=0{,}84$ en PAR. En ELN, $\bar\alpha^\ast\approx 0{,}908$ y el instrumento permanece en niveles elevados, compatible con un caso de víctima pública y baja riqueza donde el bloqueo opera más como contención estratégica que como cierre de una alta capacidad de pago. En PAR, $\bar\alpha^\ast\approx 0{,}901$ y la trayectoria permanece elevada durante gran parte del soporte, indicando que el bloqueo conserva un alto valor estratégico cuando el Estado es laxo, la familia tiene alta riqueza, y la región Oriente-Selva abre espacio para los pagos. Así, $\alpha_t^\ast$ no replica mecánicamente una rama pura: se mueve entre los puntos de referencia de Rescate y Negociación según la información acumulada, la capacidad de pago de la familia, y el valor de inducir separación entre tipos.

La precisión posterior, definida como $\iota_t=\max_{\theta\in\ThetaK}\mu_t(\theta)$, permite observar cómo cambia la presión operativa cuando el Estado gana confianza sobre la identidad del tipo verdadero. La Figura~\ref{fig:cal-eln-par-iota-gamma} reordena la simulación sobre el plano $(\iota_t,\gamma_t^\ast)$; las flechas indican el avance de $\tau$ y evitan interpretar las observaciones como una interpolación lineal. En ELN, la precisión posterior aumenta gradualmente mientras la presión operativa permanece alta pero contenida, reflejando aprendizaje sin colapso inmediato hacia una única señal dominante. En PAR, la trayectoria se concentra en una zona de alta presión y precisión creciente, consistente con un episodio en el que la información requiere una ventana más larga antes de resolver la identidad del captor.
\vspace{-4pt}
\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
...
\figurenote{\textit{Nota:} El panel (a) presenta la interdicción financiera óptima en el Escenario~I (ELN). El panel (b) presenta el mismo instrumento en el Escenario~II (PAR). La línea punteada corresponde al punto de referencia de Rescate $\alpha^R$. La banda gris marca $\tau^{\mathrm{hyp}}$.}
\end{figure}
\vspace{-6pt}
La interdicción financiera frente a la precisión posterior permite evaluar si el aprendizaje reduce o refuerza la necesidad de restringir la capacidad de pago. En el escenario ELN, reportado en la Figura~\ref{fig:cal-eln-par-iota-alpha}, $\alpha_t^\ast$ permanece alto mientras $\iota_t$ aumenta, indicando que identificar el tipo insurgente no elimina el valor de mantener la interdicción preventiva en un caso de víctima pública y baja riqueza. Algo similar ocurre en PAR: aunque el posterior acumula precisión, $\alpha_t^\ast$ permanece en niveles altos durante gran parte del soporte. Este patrón es consistente con entornos donde el Estado es laxo y la interdicción financiera conserva relevancia como instrumento de control, contención de pagos, e inducción de separación entre tipos.
\vspace{-4pt}
\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
...
\figurenote{\textit{Nota:} El panel (a) corresponde al Escenario~I (ELN) y el panel (b) al Escenario~II (PAR). Cada trayectoria muestra 20 flechas trazadas sobre intervalos homogéneos de $\tau$; el círculo identifica el punto final de la simulación, $\tau=150$.}
\end{figure}
\vspace{-6pt}
El diagnóstico de restricciones confirma que la factibilidad estratégica del mecanismo se preserva en ambos escenarios. Como muestra la Figura~\ref{fig:cal-eln-par-iric}, la restricción de incentivos del captor ($\mathrm{IC}^K$), la restricción de participación del captor ($\mathrm{IR}^K$), y la restricción de participación de la familia ($\mathrm{IR}^F$) se satisfacen en los 150 ciclos simulados en ELN y PAR. La verificación es \emph{a posteriori} respecto al tipo verdadero $\theta_K^\ast$: los instrumentos óptimos $(a_S^\ast,\alpha_t^\ast,\gamma_t^\ast)$ se evalúan directamente sobre las utilidades del tipo verdadero, sin ponderar por la creencia $\mu_t$. El mecanismo es por lo tanto factible en un sentido ex-post: dado $\theta_K^\ast$, la política óptima satisface la compatibilidad de incentivos y la participación sin abandonar el componente informacional.
\vspace{-4pt}
\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.48\linewidth}
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario II: PAR}
\end{subfigure}
\caption{Sensibilidad neta $\kappa_h(\theta_K^\ast,t)$ de la duración esperada del cautiverio respecto a $\gamma_t^\ast$ (ELN: $\tau\leq 150$; PAR: $\tau\leq 150$).}
\label{fig:cal-eln-par-kh}
\figurenote{\textit{Nota:} Cada panel traza $\kappa_h(\theta_K^\ast,t)$ para el tipo verdadero del escenario. Un valor constante de $+1$ implica $-\operatorname{sgn}(\kappa_h)=-1$ en el 100\,\% de los ciclos, es decir, $\partial\,\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast<0$: la presión operativa $\gamma_t^\ast$ \emph{reduce} la duración esperada del cautiverio en todo el soporte; la línea punteada horizontal marca el cero de referencia.}
\end{figure}
\vspace{-6pt}
Los desenlaces terminales hipotéticos difieren entre escenarios: \emph{Liberación} en ELN ($\tau^{\mathrm{hyp}}=16$) y \emph{Muerte} en PAR ($\tau^{\mathrm{hyp}}=5$). En ambos casos el posterior parte de $\mu_0(\theta^\ast)=0{,}050$ y converge a $\mu_{150}(\theta^\ast)\approx 1{,}000$, con $\bar\alpha^\ast$ y $\bar\gamma^\ast$ por encima de 0{,}86 en todos los escenarios. Estos resultados se resumen en la Tabla~\ref{tab:calibracion-resumen}.
...
\midrule
ELN & 0.050 & 1.000 & 0.908 & 0.908 & 16 & Liberación \\
PAR & 0.050 & 1.000 & 0.901 & 0.868 & 5 & Muerte \\
\bottomrule
\end{tabular}
\end{table}

En conjunto, el contraste ELN--PAR muestra que el mismo mecanismo bayesiano genera trayectorias de aprendizaje, intensidades de instrumentos, y tasas de viabilidad de restricciones distintas cuando, conjuntamente, cambian el tipo verdadero del captor y los márgenes institucional, familiar, de víctima, y geográfico descritos en el protocolo. Las simulaciones no sustituyen al Teorema~\ref{thm:posterior-consistency}, que requiere separación recurrente en el evento $\mathcal{T}$; ilustran que, con priors y hazards calibrados a la evidencia colombiana, el subsidio entrópico desplaza la política lejos de regímenes puramente miopes y acelera o disciplina la concentración posterior en horizontes finitos, en la dirección predicha por el resultado de identificación.

\section{Recomendaciones de política y conclusiones}\label{sec:conclusiones}

Esta sección integra los hallazgos empíricos y teóricos del manuscrito para formular tres recomendaciones de política económica y de diseño institucional, concluyendo con una síntesis de las contribuciones del estudio y las direcciones para investigación futura.

\subsection{Recomendaciones de política}

Los resultados de cointegración macroeconómica, los hazards microeconométricos de riesgos competitivos, y el mecanismo calibrado de control dual bayesiano respaldan conjuntamente las siguientes directrices para la política pública.

\textbf{Integración de alertas macrofinancieras en la planeación de seguridad operativa.}\quad
El análisis VEC muestra que la tasa de interés de crédito comercial y la capacidad coercitiva persistente afectan los costos de retención y financiamiento de las organizaciones criminales. En fases de contracción monetaria y restricción crediticia, el mayor costo del capital puede empujar a los actores ilegales hacia modalidades de extracción con retornos más rápidos, tales como cautiverios cortos o negociaciones aceleradas. Se recomienda integrar indicadores de liquidez, crédito formal e informal, y presión financiera regional en los sistemas de alerta temprana de las unidades GAULA. Esto permitiría anticipar cambios en el portafolio criminal y ajustar la presión operativa preventivamente antes de que las condiciones macrofinancieras se traduzcan en repuntes del secuestro extorsivo.

\textbf{Implementación adaptativa y coordinada del bloqueo financiero para evitar la colusión.}\quad
Las simulaciones muestran que el bloqueo financiero $\alpha_t^\ast$ tiene valor estratégico cuando disciplina la participación del captor y reduce el espacio para la colusión clandestina entre la familia y el captor. Sin embargo, su uso indiscriminado puede penalizar el patrimonio familiar sin aportar información adicional una vez que el posterior se concentra. Se recomienda que el Ministerio de Defensa, la UIAF, y las unidades GAULA coordinen un protocolo adaptativo que module la severidad de la interdicción según el tipo inferido, el perfil de la víctima, y la riqueza familiar. Cuando el aprendizaje bayesiano identifica al captor con precisión y reduce el valor marginal de la experimentación, el bloqueo debe suavizarse gradualmente sin abandonar la trazabilidad financiera ni la capacidad de disuasión.

\textbf{Transición hacia un protocolo de inteligencia policial basado en control dual bayesiano.}\quad
El teorema de identificación y la calibración dinámica muestran que una política puramente miope, centrada solo en la presión física inmediata, puede preservar la incertidumbre sobre el tipo y desperdiciar señales tempranas. Se recomienda formalizar el motivo informacional en los protocolos de inteligencia militar y policial, de modo que la presión operativa $\gamma_t^\ast$ busque no solo cerrar el episodio sino también separar las verosimilitudes entre organizaciones. La combinación sistemática de señales de detección, evidencia acústica, duración del cautiverio, y patrones de riesgos competitivos permitiría clasificar al captor en fases tempranas, mejorar la asignación de recursos tácticos, y reducir el riesgo humano de las operaciones.

\subsection{Conclusiones}

Este manuscrito desarrolla una arquitectura analítica para estudiar la disuasión del secuestro extorsivo en Colombia integrando teoría de la información, diseño de mecanismos, y evidencia empírica longitudinal.

A nivel empírico, el artículo muestra que el secuestro no es una serie criminal aislada. La cointegración entre $K^E$, $K^{NE}$, y $r$ revela un entorno macroeconómico persistente que condiciona los costos de retención, mientras que el MNL y los riesgos competitivos identifican heterogeneidades microeconómicas por perpetrador, perfil de víctima, región, duración, y presencia institucional. Estos resultados convierten la evidencia colombiana en priors, hazards, y restricciones utilizables por el mecanismo.

A nivel teórico, el Teorema de Identificación de Tipos Racionales bajo Entornos Desconocidos demuestra que la separación recurrente entre las leyes de señal diarias convierte el aprendizaje bayesiano en identificación posterior. Bajo un equilibrio consistente con el EBP con soporte completo inducido por el proceso MDG en la especificación base, las creencias del Estado no solo se estabilizan sino que concentran masa casi seguramente en el tipo verdadero en el evento de cautiverio prolongado ($\mathcal{T}=\{\tau=\infty\}$) cuando las huellas estructurales de los captores son observacionalmente separables.

La calibración dinámica ilustra la viabilidad operativa del mecanismo. En los escenarios ELN y PAR, la política óptima combina interdicción financiera, presión operativa, y subsidio informacional para acelerar la concentración posterior sin abandonar las restricciones de incentivos y participación. Así, el control dual bayesiano ofrece una regla práctica para equilibrar la pérdida social actual, la supervivencia esperada, y el valor del aprendizaje.

Como agenda futura, se proponen dos extensiones. La primera es generalizar el corolario de equivalencia de tendencias a estructuras longitudinales y modelos de datos panel. La segunda es extender el teorema de identificación a entornos con múltiples agentes estratégicos que interactúan simultáneamente. Esta extensión permitiría la identificación concurrente de tipos verdaderos y trasladaría el marco a otros problemas de información asimétrica, tales como la supervisión financiera, el cribado del mercado laboral, y la competencia en mercados reales.
